...
def train(encoder: str, sigma: float, seed: int, ticks: int = 4000):
    ...
    for _ in range(ticks):
        ...
        if rng.random() < EPSILON:
            ...
            rec = nac.recommend_action(agent_id="a", available_tools=ANAMES,
...
def eval_band(encoder, nac, ec, sigma, lo, hi, seed, trials=600):
    ...
    for _ in range(trials):
        gaze = float(rng.uniform(-45, 45))
        side = float(rng.choice([-1.0, 1.0]))
        rel = side * float(rng.uniform(lo, hi))
        node = None
        if encoder == "synthetic":
            rec = nac.recommend_action(agent_id="a", available_tools=ANAMES,
                                       current_drives=None, current_cluster_id=synth_bin(rel),
                                       min_confidence=MIN_CONF)
        else:
...
def main():
    ...
    print(f"  {'SYNTHETIC bins':<22}  near(trained) dir={np.mean(syn_near):.3f}   "
          f"FAR(held-out) dir={np.mean(syn_far):.3f}   transfer=0.000 (distinct keys)\n")

    print(f"  {'EC sigma':<10}{'near dir':<10}{'FAR dir':<10}{'FAR wrong':<11}"
          f"{'transfer%':<11}{'dir|matched':<12}")
    for sigma in (4.0, 8.0, 15.0, 25.0, 40.0):
        ...
        for s in seeds:
            ...
            far_d.append(d); far_w.append(w); far_t.append(t); far_c.append(c)
        print(f"  {sigma:<10.0f}{np.mean(near_d):<10.3f}{np.mean(far_d):<10.3f}"
