...
PEOPLE_TRAIN = [
    "a skier carving down the slope", "a snowboarder going downhill",
    "a person skiing on the piste", "a child sledding in the snow",
    "a woman in a red ski jacket", "a man on skis",
    "a skier doing a jump", "a snowboarder catching air",
    "a group of skiers", "a person on a snowboard",
    "a downhill skier in motion", "a teenager skiing fast",
]
PEOPLE_NOVEL = [
    "a freestyle skier mid-air", "a beginner snowboarder falling",
    "an athlete racing on skis", "a child playing in the snow",
    "a woman snowshoeing uphill", "a skier resting at the top",
    "a person in winter gear walking", "a snowboarder carving a turn",
    "a cross-country skier gliding", "a ski instructor demonstrating",
    "a person sliding down on a tube", "a mogul skier bouncing",
]
OBJ_TRAIN = [
    "a tall pine tree covered in snow", "a grey boulder by the trail",
    "a metal chairlift pylon", "a wooden trail marker sign",
    "a snow-making machine", "a wire safety fence",
    "a parked snowcat groomer", "a bare tree branch",
    "a rocky cliff face", "a lift cable overhead",
    "a plastic trail boundary post", "a mound of snow",
]
OBJ_NOVEL = [
    "a snowy evergreen forest", "a large grey rock outcrop",
    "a steel lift tower", "a yellow caution sign",
    "an idle snow gun", "a chain-link fence",
    "a piste grooming vehicle", "a fallen log",
    "an icy rock ledge", "a steel cable line",
    "a wooden fence post", "a deep snowdrift",
]
...
def main():
    ...
    def stats(P, O, label):
        ...
        across = float(np.mean(P @ O.T))
        print(f"  {label:<10} within-people={within:+.3f}  across people-objects={across:+.3f}  "
    ...
    def run(transform, threshold, seed=0, ticks=4000):
        ...
        O = {"tr": transform(ot), "nv": transform(on)}
        nac = NAc(NACConfig())
        ...
        for _ in range(ticks):
            ...
            pool = P["tr"] if kind == "person" else O["tr"]
        ...
        def p_orient(pool):
            ...
            for vec in pool:
                ...
                n += a == "orient"
            return n / len(pool)
        return p_orient(P["nv"]) - p_orient(O["nv"])     # novel discrimination
...
def best_match(ec, vec, threshold):
    e = np.array(vec); best_id, best = None, -1.0
