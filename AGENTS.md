...
- **Python API (`src/maxim/api.py`):** 17 verb-based functions, all shipped in v1.0.0. Grouped by purpose: core (`run`, `imagine`, `connect`, `diagnose`, `observe`, `configure`, `introspect`); orchestration (`campaign`, `benchmark`, `research`); extension (`on`, `register_tool`, `register_persona`, `tool`); model management (`list_models`, `download_model`, `delete_model`). All are thin facades — don't put logic in them. Heavy imports must be inside function bodies (lazy). Return structured data, never print.
