...
- `MAXIM_SIM_PLANNING_LIVENESS` = "0"/"false" opts OUT of the D13 planning-liveness recovery+abort (default ON: a planning turn that ends without an executable proposal is loudly requeued up to 3× and then aborts the sim with an `llm_wedged` finish report, instead of idling forever). Opt-in per loop via `run_agentic_loop(planning_liveness=True)` — only the sim orchestrator opts in; decision logic `runtime/loop_controller.py::LoopController.record_planning_failure` = `runtime/agent_loop.py::_handle_planning_failure`; scrub `_isolate_maxim_sim_planning_liveness_env`. **S6 apparatus note:** this changes when a campaign TERMINATES, so it is an experiment-visible control — declare it if an arm disables it.
