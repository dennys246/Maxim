...
**Future (post-Stage C3.2):** `--node install` + VRAM precheck, `--node refresh`, `/v1/mesh/*` admin API, per-agent rate limiting, request-trace ring buffer, cluster key rotation. These remained out of scope for the C3.1 + C3.2 setup-verb shipments and are tracked in [docs/plans/llm_path_operator_visibility.md](../plans/llm_path_operator_visibility.md).
