...
The `maxim substrate` verbs manage substrate snapshot bundles — ZIP archives containing the NAc causal-learning state and EC substrate nodes from a session. Bundles are how Maxim instances share learned knowledge. The 1.0 `import` verb **extracts only** — it does not auto-merge into a live system. Merging requires calling `maxim.hivemind.substrate_merge` on the extracted files (the aligned composition — hand-composing `nac_merge` + `ec_merge` is the D43 bug); a managed Oasis ingestion pipeline is gated 1.2 work.

| Command | What it does |
|---|---|
| `maxim substrate export <output.zip> --session <ID> --contributor-id <id> [--domain TAG] [--no-identity-filter] [--identity-threshold N] [--body-ref NAME] [--body-yaml PATH] [--affordance-namespace NS]` | Compose a bundle from a session's `aut_nac.json` + `aut_ec.json`. `--session` accepts either a bare session ID (resolved under `~/.maxim/sessions/{id}/`) or a path to any directory containing those files. `--contributor-id` is required and must not start with `_` (reserved namespace). `--domain` scopes the bundle to a substrate-domain tag (e.g. `"combat"`) — EC nodes outside the domain are dropped; undomained nodes ride along. Gate 7: `--body-ref` declares the body the biases were learned on; `--body-yaml` derives `capability_map` from the body's embodiment YAML (and defaults `--body-ref` to the spec name). A bundle exported without a body declaration ships `body_ref: null` and is **refused** by body-checking receivers. |
| `maxim substrate import <input.zip> --output-dir <dir> [--receiver-body NAME] [--allow-unverified-body]` | Extract the bundle ZIP to `<dir>` (created if absent). Prints the manifest summary. Does **not** merge into any running system — the extracted `nac.json` / `ec.json` must be passed through `substrate_merge` and re-loaded by the caller. Gate 7: `--receiver-body` refuses a bundle learned on a different body (`BundleBodyMismatch`) or an undeclared one (`BundleBodyUnverifiable`) before anything is written; `--allow-unverified-body` accepts an undeclared body explicitly. |
| `maxim substrate export ... --sign --license SPDX-ID [--key-file PATH] [--release-sequence N] [--signer-id ID] [--agent-id ID] [--release]` | Compose a signed **v2 release** (bundle schema 3): a detached `signature.json` over every member, with the signer, the release's place in the signing key's strictly increasing sequence (from the key's counter in `~/.maxim/util/hive_release_sequence.json`; `--release-sequence N` may only move it forward, and is required once for an existing key the counter has never seen), `--key-file` names the key (the Queen's, kept apart from this host's development key), its license (published bundles: `CDLA-Permissive-2.0`) and the per-situation entry index signed. A release ships ONE agent's NAc rows: `--agent-id` names it when the state holds several (rows under any other agent are dropped, with a count). `--release` re-authors every row as yours (Queen publishing merged contributions; requires `--sign`). Needs the `[sign]` extra. |
| `maxim substrate keygen --signer-id ID [--key-file PATH]` | Mint (if absent) and print a bundle-signing public key — this host's default key, or a named key file (public key at `<file>.pub`). |
| `maxim oasis publish <release.zip> --queen-key IDENTITY=PUBKEY_B64... [--root DIR]` | Verify a v2 release against the given Queen key(s) — signature, entry index, license, and no equivocation against a release the store holds — and publish it under its signed-payload digest. `maxim oasis serve` / `status` serve and summarise the store. |
| `maxim substrate inspect <input.zip> [--entries]` | Print the bundle manifest as JSON without extracting any files. Safe to run on untrusted bundles for inspection. `--entries` projects every situation entry of ANY bundle and prints its digest, and for a release whether the manifest's index matches the slices (a comparison, not a signature check). |
| `maxim substrate invalidate --session <ID> [--modality M --drop-geometry TAG] [--apply]` | Gate 1 migrate half. With no geometry named: print the session's per-modality geometry census. With `--modality` + `--drop-geometry`: drop EC nodes stamped with that stale geometry AND prune the NAc biases keyed on them (dry-run by default; `--apply` writes a verbatim tombstone sidecar first, then rewrites the files). Unstamped nodes are never touched. |
...
| Command | What it does |
|---|---|
| `maxim hive add <name> <url> [--queen-key IDENTITY=PUBKEY_B64]... [--domain TAG]...` | Register an Oasis and its Queen public keys (both flags repeatable) in `~/.config/maxim/hive.json`. A NEW entry refuses legacy v1-signed releases (`accept_v1: false`); re-adding keeps the entry's policy. |
| `maxim hive trust <name> [--allow-unsigned\|--require-signed] [--inherent\|--no-inherent] [--accept-v1\|--refuse-v1] [--trust-source ID \| --clear-trust-sources]` | Set that Oasis's consumer trust policy. `--accept-v1` takes the legacy v1-signed lineages (removed at 2.0). `--allow-unsigned` admits releases NOT signed by a registered Queen key, unverified; a Queen-signed release is still verified (and still refused as v1 unless `--accept-v1`). `--trust-source` is repeatable. |
| `maxim hive list` / `maxim hive remove <name>` | Show registered Oases with their policy / unregister one. |
| `maxim hive pull --from <name> --session <DIR> --receiver-body NAME [--api-key KEY] [--receiver-agent-id ID] [--release ID] [--domain TAG] [--allow-unstamped-geometry] [--apply]` | Fetch that Oasis's releases and ingest each through `substrate ingest` under the registered policy, oldest `release_sequence` first. `--receiver-agent-id` is required for a v2 release carrying memory rows (they re-key to your agent). Dry run unless `--apply`. `--api-key` is the Oasis's bearer token (defaults to the local leader key, which only serves a loopback Oasis). |
| `maxim hive contribute <bundle> --to <name> [--api-key KEY]` | Push a bundle to that Oasis's experimental tier (write-only; never read back by `pull`). |
| `maxim substrate ingest <bundle> --session <DIR> --receiver-body NAME --trust ID... [--require-signed --trust-key IDENTITY=PUBKEY_B64... [--refuse-v1]] [--receiver-agent-id ID] [--apply]` (key flags; also `--inherent-trust`, `--allow-unverified-body`, `--allow-unstamped-geometry`, `--force-digest` — see `--help`) | The ingestion pipeline `hive pull` delegates to. With `--require-signed` the session's journal also enforces release order: a second payload for an admitted `(key, sequence)` and a v1 bundle from a key whose v2 release was admitted are refused. |
