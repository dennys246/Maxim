...
| `--embodiment` | str | None | SEM component reference to load as the agent's body (e.g., `weapons/rusty_sword`, `bodies/reachy_mini`). When set, `build_executor` instantiates the entity, wraps it in `Embodiment`, and registers the component's affordance tools (e.g., `rusty_sword_slash`, `rusty_sword_parry`) into the tool registry — the agent can then invoke them via the normal tool dispatch path with the full Stage 1 pain-cascade learning. Pre-validate with `maxim doctor --embodiment <REF>`. See [embodiment_guide.md](../embodiment_guide.md) for the full flow. Mutually exclusive with `--sim` until sem_execution_hook Stage 2c lands. |
...
**Deferred to Plan 4 Stage C3 (remaining):** `--node install` + VRAM precheck, `--node refresh`, `add-node`, `remove-node`, `/v1/mesh/*` admin API, per-agent rate limiting, request-trace ring buffer, cluster key rotation.
