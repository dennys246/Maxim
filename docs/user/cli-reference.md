...
| `maxim peer --node <name> install <extras_or_packages>` | Mesh-aware install on a named node. Composes **drain → install → resume** around the shared `_install_on_target` core. Resolves the target URL + cluster key from `mesh.yml::nodes` instead of `peer.yml`. Accepts comma-separated `KNOWN_EXTRAS` (routed as `pymaxim[<extra>]`) plus raw pip package names in the same token list, matching the positional-URL `maxim peer install` verb's classifier. **Refuses self-install** (points at local `pip install pymaxim[<extras>]` instead — no `--force` flag). **Refuses a positional URL** (points at `maxim peer install <extras> <url>` as the no-mesh.yml fallback). **Was-drained sticky:** if the operator drained the node BEFORE running install, skips both the drain step AND the auto-resume — operator intent is preserved. **Leaves drained on failure:** if drain succeeds but the install itself fails, the node is left drained with a loud "STILL DRAINED → run `maxim peer --node <name> resume`" message. No auto-resume-on-failure, no silent exit. (Plan 4 Stage C3.3) |

**Future (post-Stage C3.3):** `/v1/debug/vram` admin endpoint (C3.4), `--node update` / `--node restart` (C3.5), `--node llm` (C3.6), `/v1/mesh/*` admin API, per-agent rate limiting, request-trace ring buffer, cluster key rotation, router ↔ drain-state coupling (C4 — the actual reactivity gate). Full arc tracked in [docs/plans/reactive_peer_mesh_roadmap.md](../plans/reactive_peer_mesh_roadmap.md).
