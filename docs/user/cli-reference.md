...
| `maxim substrate export ... --sign --release-sequence N --license SPDX-ID [--signer-id ID] [--agent-id ID] [--release]` | Compose a signed **v2 release** (bundle schema 3): a detached `signature.json` over every member, with the signer, the release's place in the signer's strictly increasing sequence, its license (published bundles: `CDLA-Permissive-2.0`) and the per-situation entry index signed. A release ships ONE agent's NAc rows: `--agent-id` names it when the state holds several (rows under any other agent are dropped, with a count). `--release` re-authors every row as yours (Queen publishing merged contributions; requires `--sign`). Needs the `[sign]` extra. |
