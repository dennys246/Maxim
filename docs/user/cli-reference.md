...
| `--embodiment` | str | None | SEM component reference to load as the agent's body (e.g., `weapons/rusty_sword`, `bodies/reachy_mini`). When set, `build_executor` instantiates the entity, wraps it in `Embodiment`, and registers the component's affordance tools (e.g., `rusty_sword_slash`, `rusty_sword_parry`) into the tool registry — the agent can then invoke them via the normal tool dispatch path with the full pain-cascade learning. Works with `--sim` (since 0.6): the sim orchestrator threads entity_ref to the AUT's `build_executor`, so affordance tools + pain cascade learning work in simulation. Pre-validate with `maxim doctor --embodiment <REF>`. See [embodiment_guide.md](../embodiment_guide.md) for the full flow. |
