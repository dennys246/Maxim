...
| `damage_entity` | **(Deprecated — use `damage_component`.)** Thin shim that delegates to `damage_component` with `component="torso"`. Kept for backward compat with existing orchestrator prompts. |
