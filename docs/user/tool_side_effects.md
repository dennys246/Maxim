...
| `outcome_ineffective` | `bool` — the tool RAN but accomplished nothing attributable: a motion clamped at a joint limit that moved nothing, or one whose pose readback could not confirm the target was reached. Distinct from failure (nothing was harmed) and from success (nothing happened). Keys on the OUTCOME, never on clamp-occurrence — a clamped turn that still moved toward its target does NOT set this | [`FocusOnSoundTool.execute`](../../src/maxim/tools/reachy.py) and [`ReachyOrientMotorBackend.execute`](../../src/maxim/hardware/reachy/motor_backend.py) (promoted from backend `metadata` onto `side_effects` by [`ModulatorAffordanceTool.execute`](../../src/maxim/embodiment/tool_bridge.py)) | [`runtime/tool_dispatch.py::record_outcome`](../../src/maxim/runtime/tool_dispatch.py) books `Valence.NEUTRAL` instead of `POSITIVE` — the link then falls into neither `get_positive_outcomes` nor `get_negative_outcomes`, so it contributes zero to `recommend_action`, and no goal or cluster credit is minted. [`runtime/executor.py`](../../src/maxim/runtime/executor.py) passes the same tier to [`ToolPainBridge.record_tool_complete`](../../src/maxim/bridges/tool_pain_bridge.py) | 1.1.3 |
