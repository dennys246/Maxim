...
| `drive_relief_channel` | `str` — `"exteroceptive"`: the accompanying `drive_potential_diff` is a MEASURED exteroceptive transition (post-motion sensor re-read), not a modeled delta (sem_motor_binding.md Phase 2) | [`ModulatorAffordanceTool.execute`](../../src/maxim/embodiment/tool_bridge.py) when a motor backend reports `metadata["measured_drive_transitions"]` for a live-owned drive sensor | [`runtime/tool_dispatch.py`](../../src/maxim/runtime/tool_dispatch.py) routes the ±1 cluster credit to the direction-bearing (audio/operant) cluster instead of interoception — measured exteroceptive relief is source-attributable. (Note: this credits the PRODUCTION audio-cluster space (EC node ids); the Exp 45/46 trained biases live on a disjoint bin-name key space — key unification is a named follow-up in sem_motor_binding.md) | 1.0.6 |
| `drive_progress_by_drive` | `dict[str, float]` — per-drive value-progress toward comfort, raw signed sensor units; the same terms `drive_potential_diff` sums (a test pins `sum(dict) == scalar`) | [`ModulatorAffordanceTool.execute`](../../src/maxim/embodiment/tool_bridge.py), modeled and measured paths alike — emitted even when the collateral-harm gate withholds the credit scalar | **informational / record-only.** [`runtime/executor.py`](../../src/maxim/runtime/executor.py) normalises it per drive into `ToolOutput.drive_relief` for the memory-strength encoding record (Phase 2b-ii). Deliberately NOT read by `read_learning_side_effects` — that parser is the credit path, and this is a record | unreleased |
| `outcome_valence` | `str` — `"positive"` / `"neutral"` / `"negative"`: the tool's own report of what it ACHIEVED, as distinct from mechanical success. `"neutral"` = it ran and accomplished nothing attributable (a motion that moved nothing, or one whose readback could not confirm it reached the target); `"negative"` = a CONFIRMED shortfall — a real negative outcome that is **not harm**, so it must not be routed through `embodiment_failures`. Keys on the OUTCOME, never on clamp-occurrence: a clamped motion that still moved toward its target reports nothing (POSITIVE by default). An unrecognised string is ignored, not guessed at. **Why a new key, and why three-valued** (front-gate): `drive_credit_withheld` cannot carry this — it suppresses only the CLUSTER arm, while the collapse D53 found is on the causal-link surface, which no credit-withholding flag gates; and `embodiment_failures` asserts HARM, which a refusal is not. It is a string rather than a bool because a bool has nowhere to put a confirmed shortfall — the boolean first draft of this key shipped D53's own headline case still booking POSITIVE, which the pre-merge round caught. | [`FocusOnSoundTool.execute`](../../src/maxim/tools/reachy.py) (emits all three; it returns `success=True` even on a confirmed shortfall) and [`ReachyOrientMotorBackend.execute`](../../src/maxim/hardware/reachy/motor_backend.py) (emits `"neutral"` only — it already maps a confirmed shortfall to `success=False`), promoted from backend `metadata` onto `side_effects` by [`ModulatorAffordanceTool.execute`](../../src/maxim/embodiment/tool_bridge.py) | Parsed once by [`runtime/tool_dispatch.py::read_learning_side_effects`](../../src/maxim/runtime/tool_dispatch.py) for all three dispatch paths (serial loop, `execute_parallel_actions`, `runtime/executor.py`). `record_outcome` books it as the causal `Valence` unless harm or mechanical failure dominates; a NEUTRAL link falls into neither `get_positive_outcomes` nor `get_negative_outcomes`, so it contributes zero to `recommend_action` and mints no goal or cluster credit | 1.1.3 |
