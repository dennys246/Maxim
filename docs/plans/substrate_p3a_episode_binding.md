...
- **Multi-hop retrieval via `spreading_activation` is the primary Stage 2 path** (not a fallback). `Hippocampus.retrieve_on_cue(cue, multi_hop=True)` walks the binding graph via `DependencyGraph.spreading_activation` with `EpisodeConfig.retrieval_decay / retrieval_threshold / retrieval_max_depth`. Default stays one-hop for backward compatibility — Stage 1 call sites are unaffected.
- **One-hop Hebbian ≈ TF-IDF on bag-of-words tasks (architectural invariant).** Both sit at F1 ≈ 0.70 on the Stage 2 hub+chain fixture. The Hebbian mechanism's value over bag-of-words baselines manifests specifically in multi-hop / transitive retrieval. Regression guard: `TestOneHopArchitecturalFinding::test_one_hop_does_not_beat_tfidf` locks this in with ±0.05 tolerance.
