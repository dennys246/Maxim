...
>
> **Re-audit 2026-09-21 — [paired_data_audit_reaudit_2026-09-21.md](../experiments/paired_data_audit_reaudit_2026-09-21.md)
...
- ~~**G4: Substrate-primary reward-feedback wire**~~ — **CLOSED 2026-05-11.** Track 2 deferred the cluster-keyed reward-update wire; Roy-0 caught it; this section originally documented it as an architectural gap, then shipped the wire in the same session. Summary of what landed:
  - `LLMProposal.cluster_id: str | None = None` (CC3-compatible optional field at end of frozen dataclass) — [src/maxim/agents/llm_types.py](../../src/maxim/agents/llm_types.py).
  - `propose_via_substrate` stashes the active EC cluster on the returned proposal — [src/maxim/runtime/agent_loop.py](../../src/maxim/runtime/agent_loop.py).
  - `record_outcome(..., cluster_id=...)` calls `NAc.update_cluster_reward(agent_id, cluster_id, sig, ±1.0)` when set; no-op for the `cluster_id=None` LLM-primary path — [src/maxim/runtime/tool_dispatch.py](../../src/maxim/runtime/tool_dispatch.py).
  - All 6 `_record_outcome` call sites in [agent_loop.py](../../src/maxim/runtime/agent_loop.py) plus the `execute_parallel_actions` path now thread `cluster_id` from the in-scope proposal (`ctrl.pending_proposal.cluster_id` or local `proposal.cluster_id`).
  - `NAc.dump()` / `load_state` serialize `_cluster_reward_bias` under a new `cluster_reward_bias` key (keys join `(agent, cluster, tool_sig)` with `\x1f` so `tool:use:dodge` round-trips cleanly). Backward-compatible: pre-G4 snapshots load with an empty cluster dict.
