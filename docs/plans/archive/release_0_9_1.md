...
**Companion plans:** [bio_emergent_persona_foundations.md](../deferred/bio_emergent_persona_foundations.md) (Wires 1+2+3 scoped here), [persona_convergence_crucible.md](../deferred/persona_convergence_crucible.md) (Roy iterations that motivate the release)
...
Lifted verbatim from [bio_emergent_persona_foundations.md § Stage 0](../deferred/bio_emergent_persona_foundations.md). Independent of all wires; ships ahead of them.
...
Lifted from [bio_emergent_persona_foundations.md § Wire 3](../deferred/bio_emergent_persona_foundations.md). No design changes from the foundations doc; recapped here for self-containment:
...
Lifted from [bio_emergent_persona_foundations.md § Wire 2](../deferred/bio_emergent_persona_foundations.md). No design changes; the field reservations already shipped (PR #216, 2026-04-30 under V1 Phase A). 0.9.1 implements the consumer.
...
Lifted from [bio_emergent_persona_foundations.md § Wire 1](../deferred/bio_emergent_persona_foundations.md). No design changes; `OutcomePrediction.uncertainty_interval` already reserved (PR #216). 0.9.1 implements the producer + consumer.
...
2. **"Decide whether Wire-A's render needs a raw priming snapshot floor"**: **CLOSED via tau-split + redirected to downstream gaps.** [cluster_reward_bias_decay_tau_split.md](cluster_reward_bias_decay_tau_split.md) shipped Phase 1 ([PR #267](https://github.com/dennys246/Maxim/pull/267)); Phase 3 validation in [30_wire_a_tau_validation.md](../../experiments/30_wire_a_tau_validation.md) confirmed the tau-split is structurally correct — Wire-A's annotation rendered `[strongly rewarding]` (max\|bias\| 0.753-0.997) throughout Roy-3a-retry's test arm, exactly as Phase 1 predicted. The pre-registered PRIMARY criterion (Arm A ≥1 `sense_food_source` call) still failed, but for a third reason the kickoff didn't anticipate: the substrate-scene-tool-availability gap (`sense_food_source` is a SEM-modulator-derived tool absent from the test scene's roster) compounded by imagination substrate-blindness (imagination can't dream up the missing entity because its trigger doesn't see Wire-A signals). Both gaps get their own 1.1+ plan docs: [sense_tool_registry.md](../deferred/sense_tool_registry.md) + [imagination_substrate_signals.md](../deferred/imagination_substrate_signals.md). The bias-magnitude side is no longer the bottleneck.
...
- [persona_convergence_crucible.md](../deferred/persona_convergence_crucible.md) — Roy harness; Roy-0 through Roy-2pc iteration log.
- [bio_emergent_persona_foundations.md](../deferred/bio_emergent_persona_foundations.md) — Wires 1+2+3 design (lifted here for Stages 1, 3, 4); field reservations already shipped.
