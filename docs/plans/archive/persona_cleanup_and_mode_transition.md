...
> **ARCHIVED 2026-09-19** (plans audit at the 1.3.0 release): COMPLETE — not deferrable, and it carried no revive trigger. Kept as the record of what was decided and why; not a live plan.
> **COMPLETE (2026-08-09, 1.1 deprecation cleanup):** Stage 1 shipped in PR #217 (2026-04-30). The Option A/B/C decision was resolved as **Option A** (owner-confirmed 2026-08-07), and Stages 3-6 shipped together in the 1.1 persona hard-remove PR: `simulation/personas.py` deleted (~410 LOC), `--persona`/`--sim-persona` flags and the `/persona` slash command removed, `register_persona()` raises per its deprecation promise (symbol dropped in 1.2), `SimulationResult.persona` → `.mode` with legacy-key readers (`report.json`/`session.json` "persona" accepted as alias), dispatch sites re-labeled to flow-shapes (`generative`/`dm`/`research`/`benchmark`), and all "removed in 1.1" doc promises made true. Behavior-preservation for the default `--sim` generative path was verified by a before/after sim smoke (owner's acceptance condition): byte-identical default-path orchestrator instruction, clean completion, report now carries `mode` (legacy `persona` key accepted by all disk readers). **Two recorded deviations from this plan's letter** (both review-endorsed): (1) Stage 4 said `imagine(persona=)` becomes "ignored after warning" — the implementation WARNS AND MAPS the value onto `mode` instead, because the value was always label-only and warn-and-ignore would silently relabel old callers' sessions "generative"; (2) Stage 5 said strip `register_persona` from `__all__` — the symbol is KEPT (raising) for one cycle so pre-1.1 code fails loudly with a pointer instead of an `AttributeError`, honoring stable_api.md's published "raises in 1.1" promise; it is dropped in 1.2. Kept in `deferred/` (not `archive/`) because shipped error messages and docs reference this path.

**Status:** COMPLETE — all stages shipped (Stage 1: PR #217; Stages 3-6: 1.1 hard-remove PR, 2026-08-09).
...
**Owns:** src/maxim/simulation/personas.py (DELETED in the 1.1 hard-remove), [src/maxim/cli_parser.py](../../src/maxim/cli_parser.py), [src/maxim/simulation/orchestrator.py](../../src/maxim/simulation/orchestrator.py), public API surface in [src/maxim/api.py](../../src/maxim/api.py)
