...
> **ARCHIVED (2026-07-15 plans audit; cancelled 2026-05-13 by Roy-4):** ❌ DO NOT RESURRECT. Roy-4 found zero would-have-bound edges (priming↔test EC nodes never co-fire); the resurrection condition was then empirically FALSIFIED by Exp 36 — the recognition-gap closure came from the EC drift fix (PR #264), independent of any binding edge. Cross-modal work continues via [deferred/jepa_cross_modal_alignment.md](../deferred/jepa_cross_modal_alignment.md) (a different mechanism), not this plan.


**Target version:** 1.1 (post-1.0)
**Status:** **ARCHIVED 2026-05-29 — superseded by EC drift fix.** Roy-5b-confound-isolation ([docs/experiments/36_roy_5b_confound_isolation.md](../../experiments/36_roy_5b_confound_isolation.md)) decisively falsified the resurrection rationale: the recognition gap closure attributed to the naming-event scaffold in Roy-5b is **entirely explained by the EC drift fix** (PR #264, `pattern_complete_threshold` 0.40 → 0.44, 2026-05-24). Running the Roy-4 spec at HEAD without the scaffold produced 10/10 arm-A overlap with priming — identical to Roy-5b with the scaffold. The scaffold IS structurally doing what it was designed to do (drive+linguistic co-firing produces 1 (drive, drive) binding edge at default rule that doesn't form without it) but **the recognition gap was already closed by the drift fix at the EC pattern-completion layer**, independent of any binding edges. The "Hebbian binding via temporal co-activation rescues the Roy-2c gap" framing this plan rested on is empirically dead. **Do NOT resurrect.** A new direction (if Roy-5 disambiguator's next-question surfaces a need) requires a new plan, not a revival of this one. **Note on JEPA:** the parallel [jepa_cross_modal_alignment.md](../deferred/jepa_cross_modal_alignment.md) plan stays at its pre-Roy-5b "Stage 4b candidate" status. It addresses a different problem (cross-modal alignment in different-dimensional encoder spaces) and its motivation is independent of this plan's archival or any specific Roy verdict. **Previous history:** CANCELLED by Roy-4 (2026-05-13); CONDITIONALLY ELIGIBLE for resurrection by Roy-5b (2026-05-28); ARCHIVED by Roy-5b-confound-isolation (2026-05-29). Originally written 2026-05-13.
