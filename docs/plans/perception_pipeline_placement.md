...
- [`mesh_perception_transport.md`](deferred/mesh_perception_transport.md) — the transport that moves bytes between placed stages. This plan supersedes that plan's frozen single-cut-point assumption.
...
[`mesh_perception_transport.md`](deferred/mesh_perception_transport.md) shipped its 1.0 prep with a **frozen single cut point**: "peer always segments, ships event-shaped percepts; raw frames never cross the wire." That promoted a sensible default to a near-invariant — the mirror-image of the mistake [`lane_capability_placement_split.md`](archive/lane_capability_placement_split.md) corrected for LLM lanes (conflating *what the work needs* with *where it runs*).
...
- **The observe→publish *skeleton* + percept bus** — borrow `PerceptionAgent`'s observe→classify→publish structure and the background-thread + single-worker publish ThreadPool idiom. The localized-source reading rides in `Percept.metadata` (PerceptContext is frozen), modality `"audio"`. **Caveat (don't overclaim this as drop-in reuse):** `BioEnrichmentPipeline` is *text-only* (a thalamic relay for text — keyword extraction, no slot for a scalar azimuth), and `PerceptionAgent`'s publish path is vision-detection/transcript-shaped. We do **not** reuse `CaptureManager`'s audio loop at all — since we consume the chip's DoA (a scalar), there is **no raw-audio capture** in the loop; the front-end is a small DoA-poll thread + a new audio-percept construction path. This is the substance behind Q7; it nudges the audio-path reuse below 50%.

**Genuinely new (the other ~half — must be built, not assumed):**
- **An azimuth front-end that consumes the hardware's onboard DoA.** *Hardware finding (2026-06-27):* the stock Reachy Mini's 4 mics sit behind a Seeed **XVF3800** DSP chip that exposes only a **2-channel, already-beamformed** stream — there is **no sample-aligned 4-mic raw access**, so a custom ITD/TDOA front-end is **not feasible** on this hardware (a chip-level USB limit; ALSA doesn't bypass it). Instead a background thread polls `mini.media.get_DoA()` → `(doa_radians, is_speech_detected)`, maps to a normalized `azimuth ∈ [-1,1]` (`(doa − π/2)/(π/2)`: left→−1, front/back→0, right→+1), writes it under a `threading.Lock` (RMW discipline), and a `PerceptSource.next_percept()` samples the latest value (pull-per-tick, non-blocking). **Gate updates on `is_speech_detected`** — this is the hardware's own "sound present?" flag, which directly addresses the transient-sound problem below. Full hardware analysis (DoA vs TDOA, front/back ambiguity, why elevation is impossible on this linear array) in [`docs/embodiment/reachy_mini/audio_localization.md`](../embodiment/reachy_mini/audio_localization.md). *Consequence for the thesis:* localization is done **on-chip**, so the learning is in the **orient policy** (DoA-error → head-yaw via drive-pain/NAc), not in localization — which the earlier review already judged demonstration-not-necessity anyway.
...
**First-slice success criterion:** over a session, presented with a **sustained or repeating** sound source, the agent's *discrete* orient choices increasingly reduce azimuth-error (avoid drive-pain) — learned via NAc from embodied pain, no LLM in the action path. (The head turns toward sound, in steps.) **Cost under substrate-primary (revised 2026-06-28):** the orient loop selects via `recommend_action` (no LLM in the action path — Phase 0b / [`audiovisual_orienting.md`](substrate_native_orienting.md)), so each step is a millisecond-scale NAc lookup, **not** a seconds-to-minutes LLM cycle. The loop runs at the perception/motor tick rate; the binding constraint shifts from LLM latency to DoA sample rate + motor settle time. This **relaxes** (does not fully remove) the sustained-sound requirement: a source need only persist across a few fast orient ticks, not across multi-second cognition cycles. *(Original LLM-gated framing here was written before the substrate-primary action path was adopted for the backbone.)*

> **⚠ Phase 0b correction (2026-06-28, see [`audiovisual_orienting.md`](substrate_native_orienting.md)):** strictly-pain-avoidance will **not converge as written**. Phase 0b ([`scripts/orient_backbone/phase0_affect_study.py`](../../scripts/orient_backbone/phase0_affect_study.py)) found `recommend_action` is **positive-gated** — pain (negative-only) has nothing to select on and defaults to `stay`; the causal/`record_outcome` surface is per-tool **state-blind**; and off-center pain punishes correct-but-incomplete actions mid-approach. The first slice must credit drive-pain **reduction** (relief) as a **state-conditioned positive** (`potential_diff` on `cluster_reward`), *not* the pain state. This is bio-faithful (relief = negative-reinforcement) **and** mechanically selectable, and ties to [`transition_based_drive_pain.md`](deferred/transition_based_drive_pain.md) (credit transitions, not per-tick state). Net: commit 5's "NAc credit by drive-pain reduction" phrasing is right — but the reduction must land as positive cluster reward, not be inferred from the pain surface.
...
- The blocker that cancelled [`cross_modal_substrate_binding`](archive/cross_modal_substrate_binding.md) was **temporal co-activation never occurring**, not dimensionality. Dim-consistency (both exteroceptive sensors at 384-dim *if* they share a modality space — unverified) is *necessary but not sufficient*.
...
- `cross_modal_substrate_binding` carries a cancelled/"do-not-resurrect" disposition; reviving the mechanism for a new modality pair needs a **fresh plan**, not a dimension argument. JEPA ([`jepa_cross_modal_alignment`](deferred/jepa_cross_modal_alignment.md)) is the path if a learned projection is needed.
...
4. **Cut-point payload registry — ownership.** The wire payload/envelope (how cut-point payloads are discriminated, e.g. a `cut_point` tag) is **owned by [`mesh_perception_transport.md`](deferred/mesh_perception_transport.md)**, not this plan — to avoid the "merge before multiplying" trap of two plans building the same registry. This plan owns only the cut-point *selection policy* (which boundary the placement implies); the transport plan owns what crosses it. Align the two when the transport ships.
...
6. **Commit 4:** Reachy audio front-end — background thread polling DoA (onboard: `mini.media.get_DoA()`; off-robot: poll `GET /api/state/doa` — in SDK >= 1.5 the client-side call reads local USB and only works on the robot) (NOT raw-audio TDOA — infeasible on the XVF3800; see [`docs/embodiment/reachy_mini/audio_localization.md`](../embodiment/reachy_mini/audio_localization.md)) → normalized `azimuth` sensor (locked RMW, gated on `is_speech_detected`) → `PerceptSource` → `Percept.metadata`. Reuses `PerceptionAgent`'s publish skeleton.
...
- [`mesh_perception_transport.md`](deferred/mesh_perception_transport.md) — moves bytes between placed stages; this plan supersedes its frozen-cut-point assumption.
...
- [`jepa_cross_modal_alignment.md`](deferred/jepa_cross_modal_alignment.md) / [`cross_modal_substrate_binding.md`](archive/cross_modal_substrate_binding.md) — the binding path; cancelled for temporal-co-activation reasons, not dims. A new modality pair needs a fresh plan + a co-activation measurement.
- [`lane_capability_placement_split.md`](archive/lane_capability_placement_split.md) — the capability/placement orthogonality this plan extends to perception (idioms borrowed, runtime mechanism not).
