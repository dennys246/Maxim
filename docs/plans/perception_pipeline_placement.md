...
6. **Commit 4:** Reachy audio front-end — background thread polling `mini.media.get_DoA()` (NOT raw-audio TDOA — infeasible on the XVF3800; see [`docs/embodiment/reachy_mini.md`](../embodiment/reachy_mini.md)) → normalized `azimuth` sensor (locked RMW, gated on `is_speech_detected`) → `PerceptSource` → `Percept.metadata`. Reuses `PerceptionAgent`'s publish skeleton.
