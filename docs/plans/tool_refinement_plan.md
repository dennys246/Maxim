...
Parallel to [doctor_upgrade_plan.md](doctor_upgrade_plan.md) (which curates environment diagnostics) and [multi_llm_scaling.md](multi_llm_scaling.md) (which curates compute infrastructure), this plan curates **the agent's own action + introspection surface**. Where those plans shape what humans can see, this one shapes what the *agent* can see and do.
...
| `peer_list()` | per-peer node_id, host, device, vram_gb, models, is_alive, last_latency | Phase 7c `PeerRegistry` | 🟡 |
