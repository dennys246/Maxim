...
| O9 | **`CLAUDE.md` is ~11.9k against its 12k ceiling** | `lint_claude_md_invariants.py` holds the ceiling, so the next invariant anyone adds fails CI. A diet pass is owed — but it is a judgment call about **what must stay always-loaded**, not a mechanical move: a rule demoted to a satellite doc stops being in context by default, and a guard nobody loads is a guard nobody follows. | A stated principle FIRST (e.g. "a rule stays only if violating it would be invisible without the rule in context"), then apply it. A session opener, not a closer. |
