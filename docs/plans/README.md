...
Current version: **1.1.3** (`pyproject.toml` + `src/maxim/__init__.py`; PyPI: https://pypi.org/project/pymaxim/ — `main` is ahead of PyPI by policy, CLAUDE.md §Versioning; checked by `scripts/lint_version_sync.py`). 1.1.3 = the **"Reachability"** release: D43 merge correctness + D44's behavioural gate, D62, gates 1 and 2, D61, and all three STALE graduation rows discharged. 1.1.2 = **"Decomposition"**, 1.1.1 = **"Enforcement"**. The 1.1 "Sensorimotor" line: step 5b DONE; items 17 (Exp 52) + 19 (Exp 53b) DONE; item 18 → 1.1.4.
