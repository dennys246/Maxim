...
- [substrate_p3a_episode_binding.md](substrate_p3a_episode_binding.md) — **Stages 1+2 ✅ SHIPPED** (PRs #109, #112; 2026-04-14). Stage 1: Episode dataclass + `EpisodeStore` + rule-list boundary detector + Hebbian-on-close on `Hippocampus._binding_graph` + partial-cue retrieval + 24 mechanism tests. Stage 2: hub+chain synthetic fixture + TF-IDF baseline + `Hippocampus.retrieve_on_cue(multi_hop=True)` via `spreading_activation` + `node_filter` seam + split `HebbianConfig`/`RetrievalConfig` + 10% per-seed episode dropout. Final results: **Hebbian multi-hop F1 = 0.9955 ± 0.0055 vs TF-IDF 0.6600 ± 0.0058, margin 0.324, lift over one-hop 0.3045**. Architectural finding: one-hop Hebbian ≈ TF-IDF on bag-of-words; the mechanism's value manifests specifically in multi-hop / transitive retrieval. See [../experiments/p3a_episode_binding_sweep.md](../experiments/p3a_episode_binding_sweep.md).
- [substrate_p3b_channel_integration.md](substrate_p3b_channel_integration.md) — **Stage 1 in progress** (2026-04-14). Per-channel boundary rules (SMS gap + sender change, narrative scene) via `channel_specific_rule` wrappers on the P3a rule-list seam, plus a channel-membership `node_filter` helper over the P3a inverted index. ~250 LOC + 100 metric scope — substantially smaller than P3a because the seams are already in place. Head-to-head baseline is metadata-grep (Stage 2).
...
- [sem_execution_hook.md](sem_execution_hook.md) — **NEW, parallel-safe** (2026-04-14). Companion plan for the production SEM affordance forward path discovered missing during the split audit. `tool_bridge.ModulatorAffordanceTool` wiring EXISTS but is only called from sim orchestrators (`generative_runner.py`, `dm_runtime.py`) — the standard `maxim --llm X` agent loop has no SEM embodiment startup hook. ~300-500 LOC. **Designed to ship as a parallel session** to the substrate binding track; zero file-level overlap (touches `runtime/loop_controller.py`, `cli.py`, `runtime/agent_factory.py`, `tests/substrate/test_sem_execution_production.py`). Not on the substrate version-gate path but unblocks behavioral convergence experiments (H1, H2, H4) that need real SEM execution loops.
...
- [executor_bootstrap_unification.md](executor_bootstrap_unification.md) — **DRAFT** (2026-04-14). Push the `ToolPainBridge` invariant down into `build_executor` as a required keyword arg (`pain_bus`), so forgetting to wire the bridge becomes a TypeError instead of a silent no-op. Folds in the `runtime/embodiment_bootstrap.py` helper. Triggered by three identical bug instances across `sem_execution_hook` Stages 1+2+2c — three-times-is-structural. ~550 LOC, single PR, blocks `sem_execution_hook.md` Stage 2c (which collapses to a one-line migration on top of this).
