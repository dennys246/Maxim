...
- [llm_path_refinement.md](llm_path_refinement.md) — meta-plan for the LLM routing path refactor. Motivated by two 2026-04-12 peer-leader incidents + an audit that revealed `_OpenAIBackend` has a hidden ~52s retry loop. **Ships as 0.4 stability version**, contains four focused sub-plans + three deferred shell plans. Authoritative architecture reference at [../architecture/llm_routing.md](../architecture/llm_routing.md); stress test protocol at [../experiments/protocols/llm_path_stress_test.md](../experiments/protocols/llm_path_stress_test.md).
  ...
  - [llm_path_fast_failover.md](llm_path_fast_failover.md) — **Plan 3: Fast Failover — SHIPPED 2026-04-12 (PR #94, `ce5f034`).** `_MaximPeerBackend` replaces `_OpenAIBackend` for self-hosted peers (exactly one HTTP call, no retry loop, typed `BackendError` subclasses, streaming with strict mid-stream fail). Router catches typed exceptions in specific-before-general order with per-class backoff (300s auth, 60s model_missing, 15s inference_broken, exponential for Down/Timeout, Retry-After-honoring for Overloaded). `BACKEND_CLASSES` real dispatch table wired via `resolve_backend_class` (one-line extension for new backend types). Aggregated `dispatch_exhausted` WARN + `backend_unclassified_errors_total` safety-net counter. Probe consolidation through `_MaximPeerBackend.health_check` + `for_url` factory; pre-R2.6 standalone functions kept as DEPRECATED thin compat shims to avoid ~35 test-mock migration sites (CI grep invariant allow-lists the 4 existing call sites). Three commits: R2.5, R2.6, and a pre-merge review fix that folded 12 findings from the Executor-lens + Architecture-lens review round (2 critical bugs: `for_url` env-var race + `_emit_dispatch_exhausted` bypassing the canonical `_normalize_request_context` shim). **Pre-Plan-3 baseline: ~63s leader-restart recovery (measured 2026-04-12, RTX 5080 + Mac peer). Post-Plan-3 programmatic gate: < 5s p99 against mocked-dead-peer fixture.** Real leader-restart re-measurement lives in stress protocol Phase D. 4142 fast-suite tests passing (+57 over R2 baseline). See [project_llm_path_r3_shipped.md](../../.claude/projects/-Users-dennyschaedig-Scripts-Maxim/memory/project_llm_path_r3_shipped.md) for the 10 load-bearing invariants + 5 design divergences.
  - [llm_path_cancellation_hygiene.md](llm_path_cancellation_hygiene.md) — **Plan 3.5: Cancellation Hygiene — SHIPPED 2026-04-13 (PR #96, `6a4f505`).** Inserted between Plan 3 and Plan 4 after stress test trace2 exposed a stacked-60s-timeout cascade. Establishes the **"HTTP fires first"** timeout contract: HTTP layer is authoritative (300s `_INFERENCE_PROXY_TIMEOUT_S`), agent layer is strict safety net above (300s default, was 60s). Cooperative cancellation via `threading.Event` + `ContextVar` + `contextvars.copy_context().run` propagates the cancel signal to orphan threads at backend checkpoints. `maxim/utils/cancellation.py` is the new module (was `agents/` until R6 review caught the router→agents back-edge). All five router state-mutating helpers (`_note_provider_failure`/`_overload`/`_set_long_backoff`/`_set_short_backoff`/`_note_provider_success`) check `is_cancelled()` and skip mutations on cancelled requests. 4177 fast-suite tests passing. See [project_llm_path_cancellation_hygiene_shipped.md](../../.claude/projects/-Users-dennyschaedig-Scripts-Maxim/memory/project_llm_path_cancellation_hygiene_shipped.md) for the 5 load-bearing invariants + 5 R7 deferred items + R5 sim trace evidence.
  - [llm_path_peer_failover.md](llm_path_peer_failover.md) — **Plan 3.6: Peer Failover — DRAFT v1 (2026-04-13).** The cheap concrete first step toward true mesh. Allows `peer.yml` to list multiple leaders in priority order; reuses Plan 3's typed-exception router loop for failover (no new dispatch code). ~150 LOC. Triggered by the user's RTX 5080 + RTX 3070 setup — unblocks multi-GPU testing without waiting for Plan 4's full `mesh.yml` infrastructure. Schema is intentionally a strict subset of Plan 4's `mesh.yml` so migration is trivial.
  ...
  - Deferred shell plans (revive on stress-test-defined triggers):
    - [deferred/llm_path_multi_peer_dispatch.md](deferred/llm_path_multi_peer_dispatch.md) — multi-peer reactive overflow with rendezvous-hash distribution. **Partially triggered (2026-04-13)** by the user's RTX 3070 hardware; awaiting Plan 3.6 + Plan 4 ship.
    - [deferred/llm_mesh_capability_aware.md](deferred/llm_mesh_capability_aware.md) — capability advertisement + capability-aware router ranking (NEW shell plan, 2026-04-13). Revive when ≥2 nodes serve **different** loaded models. The layer that turns the system into a true reactive mesh.
  ...
  **Long-term mesh roadmap** (current state → true reactive mesh): see the "Long-term roadmap" section in [llm_path_refinement.md](llm_path_refinement.md). Five concrete steps from leader/peer to peer-to-peer mesh with leader election, each builds on the previous, none require throwing away the current code.
...
5. **▶ NEXT:** Plan 4 (Operator Visibility) — CLI + admin API + per-agent rate limiting. Scope depends on Phase C outcome (scoped-only vs. scoped + multi-peer revival). Plan 3 infrastructure is in place: `_record_suggested_peer_hint` stub is ready for the deferred multi-peer dispatch plan to light up without refactoring router error-handling branches.
