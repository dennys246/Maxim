...
Current version: **1.2.0** (`pyproject.toml` + `src/maxim/__init__.py`; PyPI: https://pypi.org/project/pymaxim/ — `main` is ahead of PyPI by policy, CLAUDE.md §Versioning; checked by `scripts/lint_version_sync.py`). 1.2.0 = **"Oasis"**: the substrate-sharing line — **Exp 56 EARNED** (a taught want transfers between independent agents through the shipped signed-bundle ingest path; four-arm live Minecraft campaign, all gates PASS) + **Exp 57 PARTIAL** (dose-response scaling: a per-agent win, MONOTONICITY PASS, but pooling costs more total experience — NOT-JUST-MORE-DATA fail); the **Hivemind P2P exchange** end to end (Slices A–D: ed25519 signing, HTTP endpoints, `oasis`/`hive` CLIs, Queen-only trust; promotion deferred); R1/R2 pre-registered offline nulls; gates 1/3/7/8 closed. 1.1.4 = **"The world seam"**, 1.1.3 = **"Reachability"**, 1.1.2 = **"Decomposition"**, 1.1.1 = **"Enforcement"**.
...
| **1.2** | **Oasis + Hivemind** — peer substrate sharing + P2P protocol. Entry gates (encoder provenance/compatibility, read-side EC safety, bundle-version threat modeling, the 1.1 verification gates) are closed. | **Engineering DONE 2026-09-06** — headline claim EARNED ([Exp 56](../experiments/56_four_arm_sharing.md)); P2P slices A→D merged (#651–#654, [scope](hivemind_p2p_scope.md)); gates 1–6 + 8(a)(c) closed. **Remaining:** [Exp 57](../experiments/protocols/exp57_dose_response_ladder_preregistration.md) harness → Phase 0 → campaign (the second claim, may-fail), gate 8(d) Tier-3 dispositions, gate 7 status pass, release transaction. |
