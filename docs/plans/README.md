...
- [ec_centroid_drift_fix.md](ec_centroid_drift_fix.md) — **DRAFT (2026-05-23).** Phase 1-5 fix for text-modality centroid drift surfaced by [24_roy_paraphrase_diagnostic.md](../experiments/24_roy_paraphrase_diagnostic.md) (PR #259, `CENTROID_DRIFT_COLLAPSE` verdict). Sequential EC encoding pulls 19/20 paraphrase strings into one mega-node because `frozen_centroid_modalities` doesn't include `"text"`. Phase 1 ships a matrix sweep (decomp × frozen-centroid × threshold); Phase 2 regression-guards against P1+P2; Phase 3 ships the EC config change; Phase 4 validates behaviorally on Roy-2c; Phase 5 threads into V1. 0.9.1 vs 1.0 ship decision gated on Phase 4.
