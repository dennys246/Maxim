...
- [perception_pipeline_placement.md](perception_pipeline_placement.md) — **1.1 core LANDED (PRs #382–#385 all merged 2026-06-27/28)** — `runtime/perception_placement.py` (stage DAG, pinned/placeable resolver, coherence validator), exteroceptive `"audio"` EC modality, DoA audio front-end, orient affordances on `reachy_mini.yaml`. Remaining: the explicitly on-demand 1.2+ tail (leader-side segmentation / frame-transport cut point) + the pre-registered sound-localization experiment doc.
...
- [deferred/persona_cleanup_and_mode_transition.md](deferred/persona_cleanup_and_mode_transition.md) — Stage 1 SHIPPED (PR #217); Stages 2–6 (through `personas.py` hard-delete) untouched since April, blocked on the Option A/B/C testing-strategy decision. Revive when the 1.1 deprecation cycle is scheduled.
