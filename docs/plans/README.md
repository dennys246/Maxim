...
- [decay_consolidation_calibration_plan.md](decay_consolidation_calibration_plan.md) — **DRAFT (2026-05-26, 1.1+ target).** Calibration-by-simulation framework for the five NAc decay functions + (Phase 4+) the SHORT_TERM↔LONG_TERM consolidation parameters. Depends on tau-split + SCN-anchoring as prerequisites. Replaces hand-picked decay constants with tier-transition-driven calibrated defaults. Prompted by user observation during the post-Roy-3c-bisect session: "theoretically can't we ... have a particular percept occur and adjust the decay so it just begins to form a short term memory, then in another phase ... long-term memories and consolidations." Phase 0 (detailed design pass) can start anytime post-Phase-C of the tau-split chain.
- [cluster_reward_bias_decay_tau_split.md](cluster_reward_bias_decay_tau_split.md) — **DRAFT (2026-05-25).** Tier 2 follow-up to Roy-3c-bisect ([PR #266](https://github.com/dennys246/Maxim/pull/266), [29_roy_3c_bisect.md](../experiments/29_roy_3c_bisect.md)). Splits Wire-A's `_cluster_reward_bias` decay tau (currently inherits `reward_bias_decay_tau=50.0`, an inherited-by-accident value sized for EC threshold modulation) into its own field tuned for Wire-A's multi-turn substrate-voice annotation timescale (proposed default 300.0). Same pattern Wire 2's `percept_valence_decay_tau=200.0` already used. Closes Roy-3 follow-up item 2 from [23_roy_3.md](../experiments/23_roy_3.md) (was "decide raw priming snapshot floor"; tau split makes that mechanism unnecessary). Phase 3 is a Roy-3a-retry validating Wire-A becomes expressive at test time.
