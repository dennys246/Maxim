...
- [sense_tool_registry.md](sense_tool_registry.md) — **DRAFT (2026-05-26, 1.1+ target).** Surfaced by Roy-3a-retry verdict. 7 sense* tools across 3 registration regimes; LLM has no signal that inactive SEM-derived tools exist elsewhere. Sense Registry / Factory adds grayscale visibility (`[not in current location]`) without breaking auto-sense bypass or SEM scene-scoping. Complementary to [imagination_substrate_signals.md](imagination_substrate_signals.md).
- [imagination_substrate_signals.md](imagination_substrate_signals.md) — **DRAFT (2026-05-26, 1.1+ target).** Surfaced by Roy-3a-retry verdict. Imagination trigger is percept-text-bound and substrate-blind — NAc/Wire-A/ATL signals don't reach it. Three candidate hookups ranked: (1) substrate-aware scene-manifest generation (smallest), (2) per-tick subscriber for missing high-bias tools (medium), (3) arousal-gate relaxation for first-reaction-to-novel-percept (small).
- [scn_decay_anchoring.md](scn_decay_anchoring.md) — **DRAFT (2026-05-26, 1.0-or-1.1 target).** Phase C of the tau-split kickoff sequence. The five NAc decay functions currently fire per agent-loop tick (hardware-dependent rate ~10× across deployments). Migrates them to subscribe to an SCN clock-driven callback firing at wall-clock-anchored Hz. Preserves Phase A's tau=300 calibration on a representative hardware baseline. Prerequisite for [decay_consolidation_calibration_plan.md](decay_consolidation_calibration_plan.md). 1.0 vs 1.1 inclusion is an open question in the plan.
