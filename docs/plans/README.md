...
- [substrate_p3_5_persistence_snapshot.md](substrate_p3_5_persistence_snapshot.md) — **Stage 1 in progress** (2026-04-14). `BioSystemSnapshot` Protocol + `SessionSnapshot` composition across all five bio-systems. Stage 1 ships the Protocol shell + thin adapters via mechanical `_to_dict()` extraction in ATL/Hippocampus/NAc/SCN + PerceptTraceBuffer stub; Stage 2 ships real PTB save/load + migration tooling; Stage 3 ships subprocess round-trip harness + pre-merge review. Load-bearing for P3a round-trip tests and P4 (1.0-gating) mug-test subprocess boundary.
- [substrate_p3a_episode_binding.md](substrate_p3a_episode_binding.md) — **Stage 1 in progress** (2026-04-14). Episode dataclass + `EpisodeStore` + boundary detector + Hebbian-on-close + partial-cue retrieval, leveraging the existing `DependencyGraph.add_bidirectional` + `update_edge` + `get_associated` path on ATL (no new edge type). Stage 1 ships synthetic mechanism tests including the `is not None` vs truthy regression guard for the empty-ATL wire check. Stage 2 adds a 100-episode fixture + TF-IDF baseline; Stage 3 runs the ≥10-seed sweep + pre-merge review.
