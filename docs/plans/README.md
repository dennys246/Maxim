...
Current version: **1.0.0** (published on PyPI as `pymaxim`; shipped 2026-06-17).
Now: **1.1** — concurrent development track (embodiment grounding, substrate-primary validation, Oasis).
...
| **1.1** | **Embodiment grounding + substrate-primary validation** — exploration policy, substrate-primary cradle readiness (Exp 41+), perception pipeline placement (Reachy Mini), Oasis, Minecraft demo | In progress |
...
- [mesh_perception_transport.md](mesh_perception_transport.md) — **1.0 prep SHIPPED (PR #329, 2026-06-04); CORRECTED 2026-06-22.** `Percept.to_wire_dict`/`from_wire_dict` wire format; substrate fields excluded from wire; non-blocking `PerceptSource` protocol contract reserved. Full transport ships in 1.1. **The frozen single-cut-point "no raw frames" framing is superseded** by [perception_pipeline_placement.md](perception_pipeline_placement.md) — `to_wire_dict` is now one cut-point payload of several; the raw-frames ban is a default-with-opt-out.
...
- [substrate_native_orienting.md](substrate_native_orienting.md) — **ACTIVE umbrella (consolidated 2026-07-15).** The authoritative plan for the orient-to-center line: the substrate learning its first **real-hardware sensorimotor policy** (turn to drive bearing-error → 0, credited by drive-pain *reduction* via NAc). Absorbs the former `audiovisual_orienting.md`; sits above the [`reachy_orient_live.md`](reachy_orient_live.md) hardware runbook (Step 1 PASSED 2026-07-15) and the [`perception_pipeline_placement.md`](perception_pipeline_placement.md) infra. Two-layer framing: **Layer 1** = the orient policy (achievable, hardware-validated); **Layer 2** = spatial co-location as the grounding substrate for language + vision (the "words + visuals to places in space" north star, gated). Carries the learned-vs-servo rigor bar.
- [reachy_orient_live.md](reachy_orient_live.md) — **ACTIVE hardware runbook (Step 1 PASSED 2026-07-15).** Live device bring-up of the orient-to-center backbone on a physical Reachy Mini (WS-era SDK ≥1.5). Steps 2-3 (reactive orient / learning loop) next. Execution layer under [substrate_native_orienting.md](substrate_native_orienting.md).

- [perception_pipeline_placement.md](perception_pipeline_placement.md) — **DRAFT (2026-06-22, 1.1 abstraction + first cut point; corrected after a code-grounded review).** Per-stage workload placement for the perception pipeline — the capability/placement orthogonality from [lane_capability_placement_split.md](lane_capability_placement_split.md) extended to perception, *borrowing its config/type idioms but NOT its runtime composition* (perception is a pipeline, not a failover list; some stages are pinned-by-physics). Records the correction to [mesh_perception_transport.md](mesh_perception_transport.md)'s frozen single-cut-point assumption. Driving consumer: a **Reachy Mini sound-localization cradle** (4-mic ITD → discrete head-orient learned via NAc from drive-pain). Honest accounting: first slice is ~50% reuse (drive/pain/NAc) + ~50% genuinely-new (real-time audio front-end, exteroceptive substrate modality, orient affordances); cerebellum forward-model is dormant and explicitly out of the first slice; audio↔visual binding is a gated open question (Roy-4 cancelled binding for temporal-co-activation reasons, not dims).
...
Recently archived (2026-07-15 — plans consolidation + shipped/concluded sweep):
