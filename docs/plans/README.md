...
- [behavioral_graduation_candidates.md](behavioral_graduation_candidates.md) — **1.0 gate, paired alongside benchmarking** (2026-05-27 v0.1). Tracks which `[engineering]` invariants from CLAUDE.md need to graduate to `[behavioral]` via cited Roy/equivalent experiments before 1.0 ships. Three-tier scope (thesis-load-bearing must graduate, scaffolding stays engineering, middle subset graduate-or-downgrade). v0.1 seeds 20 candidates; honest accounting flags 4 as "can't predicate yet — claim fuzzy" — those are the most useful finding (bio-framing outrunning experimental backing).
