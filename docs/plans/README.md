...
Current version: **1.1.4** (`pyproject.toml` + `src/maxim/__init__.py`; PyPI: https://pypi.org/project/pymaxim/ — `main` is ahead of PyPI by policy, CLAUDE.md §Versioning; checked by `scripts/lint_version_sync.py`). 1.1.4 = **"The world seam"**: Minecraft bridge + 16-sensor player body + world channel + two-AUT harness + A4 gain (world-only, unchanged 0.85 threshold) behind the vectorized exact scan; L11 re-measured on live world data — `mitigation-confirmed`, NOT retired (L11 stays ACTIVE); console bearer auth + Reachy device handoff (A9). 1.1.3 = **"Reachability"**, 1.1.2 = **"Decomposition"**, 1.1.1 = **"Enforcement"**. The 1.1 "Sensorimotor" line: items 17, 18, 19 all DONE.
