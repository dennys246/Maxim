...
|---|---|---|---|---|
| **0a** | **Multi-URL `peer.yml`** with priority-order failover. Reuses Plan 3's typed-exception router loop. ~150 LOC. | [llm_path_peer_failover.md](llm_path_peer_failover.md) (Plan 3.6) | User has a hot-standby leader (RTX 3070 alongside RTX 5080) | ▶ READY (Draft v1) |
...
| **2** | **Capability advertisement** — each node exposes `loaded_model`, `vram_free_gb`, `tier`, `tokens_per_sec`. Router caches + ranks providers by capability. ~300 LOC. | [deferred/llm_mesh_capability_aware.md](deferred/llm_mesh_capability_aware.md) | ≥2 nodes with **different** loaded models | ⏸ Shell plan exists |
