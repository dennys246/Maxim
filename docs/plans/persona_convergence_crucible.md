...
- **G4: root-caused, documented as architectural gap, NOT a Roy-harness bug.** Three parallel investigations (static gate trace + commit forensics + persisted-state inspection of Roy-0 artifacts at `~/.maxim/sim_reports/20260510_213337/` and `213527/`) converge on the same finding: the cluster-keyed reward-update wire was **explicitly deferred** when cluster-keyed action selection shipped in commit `6d0e4a7` (Track 2 of the substrate-primary series). The commit message states: *"Reward update wiring (cluster_id-aware `record_outcome`) is deliberately out of Track 2's scope — the API exists and is unit-covered, but the agent_loop's `_record_outcome` chain doesn't yet plumb cluster_id."* Consequence: `NAc.reward_bias` stays empty (0 keys after 15 min on every arm), `NAc._cluster_reward_bias` stays empty, and `NAc.recommend_action` returns None at the [nac.py:1295/1300](../../src/maxim/decisions/nac.py) score-threshold gate every tick. The 133 causal links and 662 episodes Roy-0 reported are *passive observation accumulation* — the learning loop never closed because no actions fired. **Full architectural-gap writeup** with the wired-vs-deferred ledger, the gate trace, the persisted-state evidence, and the cost/scope of closing the wire is now in [grounded_language_acquisition.md § Phase 0 → "G4: Substrate-primary reward-feedback wire"](grounded_language_acquisition.md). The empirical "Substrate divergence" numbers above (`reward_bias_l2 = 0.0000` × 3 pairs, valence KS = 0 × 3 pairs) now have a documented root cause: they reflect the absence of cluster_id in the outcome-recording chain, not anything wrong with R1/R2/R3/R4. **Implication for Roy-1:** Roy-1 should not run on substrate-primary until G4 closes (a sub-Roy-0 with `reward_bias_l2 > 0` on the A-vs-blank pair would confirm). Alternative: run Roy-1 on `aut_mode=llm-primary` to validate the persona-convergence methodology against a working AUT proposer while substrate-primary's wire is fixed in parallel — strategic call deferred to the user.

**What to change before Roy-1 (concrete next steps, prioritised):**

1. **G4 (blocking — substrate-primary track):** close the cluster_id reward-feedback wire per the [Phase 0 writeup](grounded_language_acquisition.md). Estimate 1-2 days threading + invariant tests + a sub-Roy-0 re-run to confirm `reward_bias_l2 > 0` on A-vs-blank. Until then, Roy-1 on substrate-primary will reproduce Roy-0's all-zero substrate divergence.
...
3. **Roy-1 AUT-mode decision:** with G4 open on substrate-primary, run Roy-1 on `aut_mode=llm-primary` to validate the persona-convergence methodology end-to-end while the substrate wire is fixed in parallel — OR wait for G4 to close. Strategic call.
