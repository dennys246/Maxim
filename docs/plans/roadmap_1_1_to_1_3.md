...
> **HISTORICAL as of 2026-09-19.** This was the sole scope authority through 1.2; the live sequence
> is [roadmap_1_3_x.md](roadmap_1_3_x.md) → [roadmap_1_4.md](roadmap_1_4.md). Kept because other
> plans cite its item numbering (1.1.x items 1–16, the pre-1.2 gates 1–8) and for the release table.
> Its title no longer describes its contents.
...
| **1.2** | **Oasis + Hivemind** | ~1,400 LOC of de-risked engineering with a cleared gate — **motivating case study adopted 2026-08-26: sharing the nursery-taught want ([oasis_case_study_taught_orient.md](archive/oasis_case_study_taught_orient.md)); the claim ladder ends at cross-unit readout on a second Reachy** | Low-medium. Known shape. |
| **1.3** | **"Oasis-2" — the survival world** *(rescoped 2026-09-09)* | Learning reward FROM the game: the survival loop (R2's 3 breaks), the survival world with marked classrooms (dining hall, dark=danger, wolf defense) **+ the survival reflex tier** (startle/flee below deliberation — the innate fast layer that buys time for the slow learned one), and the headline — **shared *survival* wants** (an agent shares a hard-learned survival want; the receiver survives its first night). Phased plan: [roadmap_1_3.md](roadmap_1_3.md); designs: [survival_world_1_3.md](archive/survival_world_1_3.md) + [intrinsic_motivation_1_3.md](deferred/intrinsic_motivation_1_3.md) | **High — pivotal may-fail (shared-survival-want / R3 benchmark).** |
| **1.4** | **Working title "Anticipation"** — re-pointed 2026-09-18 ([roadmap_1_4.md](roadmap_1_4.md)); *"Shared perception" DEFERRED on a physical trigger (a second body), plan intact* | Richer video/audio (cochlear front-end, vision encoder, binding, three-factor calibration) **plus sharing at the perception layer**, the **DN-canonical *orienting* reflex** (perception-coupled — stays here, distinct from 1.3's survival reflex), **the microduck** ([microduck_intent_layer.md](deferred/microduck_intent_layer.md)), **Exp 55** (aversion/coding-habit transfer), and **breeding** (deferred from 1.3). Sequencing: [roadmap_1_3_path.md](roadmap_1_3_path.md) (now the 1.4 plan) | **High — pivotal may-fail (Stage 0c); shared-perception inherits R1's transfer bound.** |

> **RESCOPE 2026-09-09 (owner decision): 1.3 = the survival world; the perception fabric +
> microduck + Exp 55 move to 1.4.** After 1.2 "Oasis" shipped (Exp 56 EARNED + Exp 57 PARTIAL),
> the survival-world line ([survival_world_1_3.md](archive/survival_world_1_3.md), designed post-1.2)
> became the natural thesis continuation — 1.2 shared a *taught* want; 1.3 shares a *survival*
> want. It is software-only (Minecraft, no hardware gating — a real advantage given the Reachy
> hardware friction) and rides the Oasis momentum. The perception fabric + microduck keep their
> deliberate "may-fail far" character, one minor further out at 1.4. The "Why this ordering"
> rationale below is retained as the historical 1.1→1.2→1.3(perception) reasoning; its
> put-the-may-fail-research-far principle still holds — it now governs 1.3's survival benchmark
> and 1.4's Stage 0c alike. This doc's title still reads "1.1 → 1.3"; it now extends through 1.4
> (filename kept to avoid a link cascade).
>
> **2026-09-18 (owner):** 1.4 re-pointed to generalization + anticipation on the survival rig ([roadmap_1_4.md](roadmap_1_4.md), five-lens reviewed); "Shared perception" DEFERRED on a physical trigger — it revives the day a second body exists, Stage A unchanged.
...
2. **Respect the plans' own stated targets and published commitments.**
   `deferred/cross_modal_perception_fabric.md` and `three_factor_credit_assignment.md` both
...
2. **Annotation S4 — non-stationarity analysis** (`deferred/annotation_context_and_provenance.md`).
...
2. **Decision provenance Stages 3+4** (`deferred/decision_provenance.md`): make the provenance
   fields queryable + wire them into the S2 apparatus canaries. Stage 4 overlaps the
   canary work — build together.
3. **Annotation S2 (context-aware view) + Exp 51** (`deferred/annotation_context_and_provenance.md`):
...
5. **Place-code default-ON** (`deferred/modality_resolution_and_alignment.md` §7). Gates: D2
...
6. **Fail-loud Stages 2–3** (`deferred/measurement_path_fail_loud.md`): read the Stage-1
...
7. **God-function decomposition** (`archive/god_function_decomposition.md`) — its own plan
...
10. **Memory-consolidation decision** — `archive/memory_consolidation_practice.md`'s own
...
15. **Reachy-native nursery body — Exp 54 [Phase A GRADUATE 2026-08-27](../experiments/54_nurture_reachy_body.md) (harness #561; taught 0.858 / satiated 0.472 / no_feed 0.514 on `bodies/reachy_mini_infant`; Phase B/C on the robot pending — item DONE only when all three phases are recorded); pre-registered 2026-08-26 as [Exp 54](../experiments/protocols/exp54_nurture_reachy_body_preregistration.md)** (added 2026-08-26, from Exp 53b + the Oasis case
    study [oasis_case_study_taught_orient.md](archive/oasis_case_study_taught_orient.md)): an
...
16. **Scorecard mechanization** (added 2026-08-27; every piece rides an existing lint or
    ...
    6. **DONE 2026-08-29 (1.1.1 Cluster C: rows 1/2/6/7 re-measured — row 7's CI claim had been false since #527, row 6's diet had shipped, row 2's Stage-4 lock had shipped; every row now carries the date its numbers were measured) — Truth pass on [external_critique_response.md](archive/external_critique_response.md)**
...
|---|---|---|---|
| **1.1.2** | **Decomposition** | fail-loud **Stage 2** (never run — it is the extraction's own stated prerequisite, and the per-PR behaviour gate cites a baseline that does not exist); the first `run_agentic_loop` extraction; the four cheap scorecard conditions (Tier-3 dispositions, `test_api_surface.py`, ARCHITECTURE.md EC rows, item 16.10); the D12 `pytest-timeout` guard; a scheduled `-m slow` lane | The length baseline tightens in the same commit, AND the extraction passes the corrected behaviour gate in [god_function_decomposition.md](archive/god_function_decomposition.md) §"Behavior-preservation gate per PR". **Revised 2026-08-30 by measurement:** the original wording — a JSONL sequence-diff against a pre-refactor capture on the same seed — is UNSATISFIABLE. The generative sim is not reproducible on identical code; three runs of one command, *two of them both pre-extraction*, gave `percept` counts of 51, 8 and 0, so the two pre-extraction runs differ from each other more than the post-extraction run differs from either. Held to the letter it fails every mechanical extraction; held loosely it means nothing. What replaces it: the **substrate probe** is the sequence-diff vehicle (no LLM, bit-reproducible — byte-identical 72,239-record captures before and after the first extraction, digests committed), the generative sim supports a **structural** comparison only, and the swallow gate (`fail_loud_stage2.py check`) plus per-section direct unit tests carry the weight for an agent-loop extraction, since the substrate probe never enters `run_agentic_loop`. |
| **1.1.3** | **Merge correctness** | ~~**D43** both halves (id map out of `ec_merge`; re-key `cluster_reward_bias` + `cluster_reward_source`; N→1 fold semantics; a bias-key identity namespace — the undocumented design gap)~~ **SHIPPED 2026-09-02** (#590 + #591) — five barriers closed, incl. the per-modality threshold no plan document named. **#591 is the lesson**: #590 shipped the pieces and left the COMPOSITION to call sites, of which there were zero, so the defect stayed live behind a green suite; caught only because D44 was `xfail(strict=True)` and did NOT flip ([shipped-the-pieces-not-the-composition.md](../lessons/shipped-the-pieces-not-the-composition.md)); ~~gate 1 encoder-provenance validator~~ **SHIPPED 2026-09-03** (#597) — D1's complaint was that nothing READ the stamp; `pattern_complete_or_separate` now takes the query's `geometry` and skips nodes declaring a different space, skip-and-warn (deduped per triple) rather than raise, and stamps on first touch so the guard is not inert for existing installs. The migrate half of "reject **or migrate**" is NOT shipped — an `ec invalidate` / re-encode path is owed; ~~gate 2 geometry tag + threshold pin~~ **SHIPPED 2026-09-03** (#596) — D3 pins the duplicated thresholds against `SensorEncoderConfig`/`ECConfig` (D3's own trigger was the per-modality thresholds D43 shipped), D4 gives EC nodes a `geometry` tag so a SAME-dimension encoding change cannot fold invisibly. **Both were hardened by a three-lens pre-merge round that returned two cross-confirmed BLOCKERs** (the tag named the READING not the SPACE, so a drive crossing its comfort band forked the cluster space; and `geometry` was an optional kwarg a live caller had already omitted) — see #598; ~~gate 7 typed bundles (`body_ref` + `affordance_namespace`)~~ **SHIPPED 2026-09-01** — option (a), plus a `capability_map` emitted alongside so a later capability namespace is a reader-side change with no migration ([d43_merge_correctness.md](archive/d43_merge_correctness.md) §5a) | ~~**D44**: a test asserting a *behavioural* delta across a merge — `recommend_action` changes — between two genuinely independent agents. Dict equality does not count~~ **MET 2026-09-02** (#589 + #591) — `tests/unit/test_d44_merge_behavioural_delta.py`, 10 arms, **0.0 → 1.0** on the shipped path with 4 of 4 merged keys reachable (was 0 of 4) |
| **1.1.4** | **The world seam** | The Minecraft bridge, `bodies/minecraft_player.yaml`, the world modality channel **plus its selection-dynamics re-baseline**, the two-AUT-one-world harness ([minecraft_benchmark.md](minecraft_benchmark.md)). **Added 2026-09-01, CORRECTED 2026-09-03 — read this before implementing:** the sensor-encoding change is the **nonlinear gain (arm A4)** at the **UNCHANGED 0.85 threshold**, weighting each sensor's contribution by its distance from set point. The text this replaces prescribed ~~per-type channels plus a sensor-count-scaled `pattern_threshold`~~, written hours before the bake-off measured that exact combination as the **worst** arm tried: threshold alone scores 0.70–0.84 degrading with N, grouping alone is near-useless at scale (0.00 at N=100), and the two **together** are worse than the threshold alone (0.62 vs 0.76 at N=50) because grouping shrinks per-channel N, which loosens `1 − k/N` and lets noise separate. A4 scores **1.00/1.00/1.00 from N=30 up**. Building from the superseded sentence would reproduce a measured-worse design — the D43 §5 failure mode, where a roadmap line instructs an implementer to rebuild a known barrier. **The scan-cost prerequisite (mis-filed as D51; corrected 2026-09-03, see the D51 ledger row):** A4 allocates ~120× the control's clusters against an EC substrate-node scan that is exact `O(N_nodes · d)` with no cap, pruning or index — `LSHIndex` (D51) is not on that path; the prerequisite measurement + frozen decision rule live in `scripts/ec_scan_cost.py` ([world_seam_1_1_4.md](archive/world_seam_1_1_4.md) decision D4). A `modality:` declaration on the sensor schema is still worth doing — but for its own reason (a sensor should declare its modality rather than channel membership living in hardcoded name tuples), **not** as this limit's mitigation. **NARROWED 2026-09-03 (PR 1, plan D3/D6 — see [world_seam_1_1_4.md](archive/world_seam_1_1_4.md)): A4 shipped PER-MODALITY, world-only** — at N=6 the bake-off's own instrument measured A4's stability collapsing 0.97→0.62, so interoception and audio stay in the byte-identical pre-A4 space and **Exp 53b does NOT re-stale** (fired triggers discharged with dated ledger annotations; a later flip of an existing modality into `gain_modalities` re-stales it). The hardware block is now L11's post-mitigation re-measure + the owed roll/pitch recalibration. Evidence: [l11_sensor_dilution.md](../limits/l11_sensor_dilution.md) §Bake-off** ([minecraft_benchmark.md](minecraft_benchmark.md) §"The sensor ceiling is a THRESHOLD artifact"). | Infrastructure only, **no claim**. Smoke benchmark green; `is_sim_mode=False` verified to consolidate. **GATE DISCHARGED 2026-09-04 (released 1.1.4):** smoke green + non-vacuous in fast CI (liveness-gated `verdict_is_green`, dead-feed negative control) AND on the live Paper 1.16.5 server (`docs/experiments/data/minecraft_live_smoke_2026-09-04.json` — first run honestly RED on a static world, disclosed; green after hostiles), consolidation verified live (world nodes persisted per AUT). The L11 re-measure ran per the frozen protocol: **`mitigation-confirmed`, NOT retired-eligible** (`docs/experiments/data/l11_remeasure_verdict_2026-09-04.json`) — L11 stays ACTIVE, A4 partial mitigation. **The encoding change ships HERE and not in 1.2, deliberately** — see the note below |
| **1.2** | **Oasis** | The four-arm sharing benchmark, pre-registered, run in Minecraft at n ≥ 50, replicated on two Reachy Minis at n = 12; gates 3, 4 and 8. **Plus the poison-resistance slice (added 2026-09-05, instrument + mechanism, NO claim):** tighten-only merge + the inherent bias class riding the V1–V10 ingestion adapter, and **Gauntlet #3** (coding-safety battery) as the Queen-promotion instrument — [coding_habits_oasis.md](deferred/coding_habits_oasis.md) §4; sequencing in §"The 1.2 poison-resistance slice" below. The ship gate is unchanged by this addition | The gate ladder below |
| **1.3** | **"Oasis-2" — the survival world** (rescoped 2026-09-09) | The survival loop (R2's 3 breaks) + the survival world with marked classrooms + **the survival reflex tier** (Phase 1b — innate startle/flee below deliberation, gated `[engineering]`) + the headline **shared survival wants**. R1 (generalization) and R4 (delayed credit) are the frontier mechanics gating the rich classrooms (crafting, farming, wolf upkeep). Full phased plan + dependency markers: **[roadmap_1_3.md](roadmap_1_3.md)**; designs: [survival_world_1_3.md](archive/survival_world_1_3.md), [intrinsic_motivation_1_3.md](deferred/intrinsic_motivation_1_3.md); benchmark Part II: [minecraft_benchmark.md](minecraft_benchmark.md) | Pivotal may-fail (shared-survival-want / R3 survival benchmark). Platform: modern MC (1.20.1), re-verify sensors first. |
| **1.4** | **Working title "Anticipation"** — re-pointed 2026-09-18 ([roadmap_1_4.md](roadmap_1_4.md)); "Shared perception" DEFERRED on a physical trigger (a second body), plan intact | Richer video/audio — cochlear front-end, vision encoder, binding, three-factor calibration — **plus sharing at the perception layer**, the **DN-canonical *orienting* reflex** (perception-coupled; stays here, distinct from 1.3's survival reflex), **the microduck** ([microduck_intent_layer.md](deferred/microduck_intent_layer.md)), **Exp 55** ([coding_habits_oasis.md](deferred/coding_habits_oasis.md) §6), and **breeding** (deferred from 1.3) | Its own pivotal experiment (Stage 0c); shared-perception inherits R1's transfer bound. **SEQUENCING: [roadmap_1_3_path.md](roadmap_1_3_path.md)** — Stage A duck baseline → Stage B engine seam / robot factory → Stage C fabric |

> Instrument limit: **[L11](../limits/README.md#l11--sensor-count-dilution-and-the-discrimination-ceiling-behind-it--mitigated)** (dilution + discrimination ceiling). The 1.1.4 work IS L11's stated mitigation; L11 moves to `RETIRED` only when it is shipped AND re-measured. **Re-measured 2026-09-04 (pre-registered, live world, N=16): `mitigation-confirmed`, NOT retired-eligible — A0 fully blind (0.0), A4 separates (0.0566 « the 0.70 bar). L11 stays ACTIVE; the scaled threshold remains the retirement path.**
>
...
>
> **Hardware batching (superseded twice — the A4 correction, then PR 1's world-only narrowing; the shipped A4 does not re-stale 53b):** grouping + threshold re-stales Exp 53b on the Reachy, and 1.2 separately
...
### The 1.2 poison-resistance slice — adopted 2026-09-05 (instrument + mechanism, NO claim)

[coding_habits_oasis.md](deferred/coding_habits_oasis.md) is **named by this item**. What 1.2 gets:
the tighten-only merge semantics and the inherent bias class (Queen-promoted,
decay-exempt, tighten-only — plan §4), and **Gauntlet #3**, the coding-safety battery, as
the Queen-tier promotion instrument. What 1.2 does **not** get: any behavioral claim
about habit transfer — that is **Exp 55**, held to the 1.3 row of the ladder above, per
the one-frozen-confirmatory-test rule and enthusiasm-flag #3. The 1.2 ship gate is
unchanged by this adoption; the four-arm claim set stays exactly as frozen.

Two sequencing decisions, decided 2026-09-05:

1. **Tighten-only merge lands WITH the V1–V10 ingestion adapter** — same seams
   (`validate_link` / `trusted_sources`), one review round, and the four-arm prereg then
   freezes on the real 1.2 merge semantics rather than a version already scheduled for
   replacement. The clamp is sign-scoped: a NEGATIVE bias may deepen but is never raised
   toward zero by an import, and **positive-bias folds are untouched** — the taught-want
   arms merge positive valence, and the adapter's review round checks that property
   explicitly (a tighten-only clamp that leaks into positive folds would silently change
   the benchmark's arm 2).
2. **Slice 1 (code-world infrastructure: scenario-conditional coding tools,
   `SandboxExecutor` wiring, the `PainTriggerLayer` disposition, `CodeSandboxBackend` +
   the coding body) merges only AFTER the four-arm data is captured.** Branch work may
   proceed in parallel; merge order is the discipline — no new `src/` lands on the
   shared subsystems under a pre-registered campaign.

**1.2 build order with this folded in:** V1–V10 ingestion adapter (+ tighten-only +
inherent bias class) → item 15 nursery body → four-arm benchmark → cross-unit readout →
Slice 1 code-world infra + Gauntlet #3 battery.
...
> through against the code in **[microduck_intent_layer.md](deferred/microduck_intent_layer.md)**. The
...
design** ([microduck_intent_layer.md](deferred/microduck_intent_layer.md) §1.1, §5, §8 items 5–6). The
...
7. **Bundle action namespace** (added 2026-08-26; **DECIDED 2026-09-01, reconciled
   ...
   [oasis_case_study_taught_orient.md](archive/oasis_case_study_taught_orient.md) §1 are now the
   SAME answer by construction; the full costing record is
   [d43_merge_correctness.md](archive/d43_merge_correctness.md) §5/§5a, which is where the
...
- **Real scope is ~900–1,400 production LOC**, with an unresolved design decision
  (reflex NAc isolation) and a *mandatory unbuilt prerequisite* (`deferred/sem_motor_binding.md`:
