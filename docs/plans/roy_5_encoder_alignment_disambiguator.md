...
**Status:** **Stage 3 COMPLETE 2026-05-28 — Conditional PASS / Ambiguous (surfaced to user). Stage 4a ELIGIBLE pending two gates; Stage 4b parks at "candidate."** Roy-5b ([docs/experiments/35_roy_5b.md](../experiments/35_roy_5b.md)) produced 1 matching priming↔test edge at the default rule (`min_cofire=5, min_weight=0.5`) — both endpoints are priming `sense_food_source` clusters — but the matched edge's modality_pair is **(drive, drive) intra-modal**, NOT the cross-modal (drive, linguistic) binding the naming-event scaffold was designed to enable. Cross-modal edges DO form at sub-default thresholds (`min_cofire ∈ {1, 2, 3}`) but drop out at default. The disambiguator plan's pre-registration table row 1 (literal PASS) AND row 3 (Ambiguous: structural shape not matching the binary diagnostic) are both triggered. Two-lens reviewers (architecture + bio-fidelity) independently classified as Ambiguous; per the kickoff's "force into 4a or 4b only when unambiguous" rule the verdict surfaces to user. **Stage 4a eligibility is gated on:** (1) confound-isolation re-run (Roy-4 spec at HEAD with `MAXIM_SUBSTRATE_PATH=1` and standard `infant_humanoid` body — isolates whether the 0% → 100% recognition gap closure is scaffold-driven, threshold-driven, or both, given PR #264 raised `pattern_complete_threshold` 0.40 → 0.44 between Roy-4 and Roy-5b); (2) user classification call on whether the (drive, drive) intra-modal edge counts as evidence the cross-modal binding mechanism is rescued. Stage 1 (Roy-5a cosine localization, 2026-05-13) confirmed H1a; the exp 30→32→33→34 sequence proved the substrate→action thesis is bottlenecked at the encoder layer.
...
Resurrect `docs/plans/cross_modal_substrate_binding.md` with:
...
- [docs/plans/cross_modal_substrate_binding.md](cross_modal_substrate_binding.md) — cancelled by Roy-4; resurrectable by Stage 4a (conditional on H1a + Stage 3 PASS).
...
- [docs/plans/release_0_9_1.md](release_0_9_1.md) — unchanged by this plan; Wire-A remains the operator-visible interim.
