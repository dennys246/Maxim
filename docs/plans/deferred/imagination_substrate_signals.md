...
> **DEFERRED (2026-07-15 plans audit):** Hookup 1 MVP SHIPPED (PR #288 + Fix B PR #292 — substrate-aware manifest via `nac_top_biases`; note the universal `MAXIM_DISABLE_IMAGINATION` gate now sits above the signal-specific env var). Hookups 2 (per-tick subscriber) + 3 (arousal-gate relaxation) unbuilt — deliberately gated on a Roy verdict that instead pointed at encoder replacement. **Revive when:** a post-encoder Roy iteration shows Hookup 1 closes part of the gap and per-tick substrate-driven imagination is the next lever, or Open Question 5's self-reinforcing-preference risk becomes load-bearing on the fixture path.
...
**Integration test outcome (2026-05-27, [32_wire_a_post_w1_w2.md](../../experiments/32_wire_a_post_w1_w2.md)):** AMBIGUOUS-WITH-WIRING-BUG. The W2 MVP code is structurally correct for its named precedent (cradle's generative-narrator manifest pre-trigger). The integration test surfaced that Roy's test arms use the `roy_1_holdout.yaml` fixture, which bypasses `generate_scene_manifest` entirely — W2's hookup site at [orchestrator.py:1468](../../src/maxim/simulation/orchestrator.py) is never reached during the test arm where the substrate-aware manifest would matter. The 10 `narrator_generation` events in this run are all from priming. **W2's MVP fires correctly for the cradle precedent it was scoped against; Roy's fixture-driven test arms are structurally outside that precedent.** Two fix shapes scoped in the experiment doc (Fix B): (b1) extend W2 hookup to fire on fixture scene-load, or (b2) Roy iterations switch test arms to a generative-narrator path. (b1) bumps directly into Open Question 5 (self-reinforcing preference loops); (b2) abandons fixture-driven reproducibility. Surface decision to user before next iteration.

**Post-Fix-A re-run confirmation (2026-05-27, [33_wire_a_post_fix_a.md](../../experiments/33_wire_a_post_fix_a.md)):** with Fix A landed and Wire-A demonstrably reaching the LLM at `[strongly rewarding from prior experience]` magnitude throughout Arm A, PRIMARY still failed (Arm A=0 `sense_food_source`). The load-bearing follow-up cause is unchanged: `sense_food_source` is not in the active tool roster because no food entity is in the fixture scene. **W2's Bug B is now confirmed as the next-iteration test gate** — the divergence-in-a-row trigger is met formally, but bringing a food entity into Arm A's scene via Fix B (extend W2 hookup to fixture scene-load) is the cheaper, more-bio-fidelity-conservative test before committing to encoder replacement (Roy-5a Stage 3 + JEPA, 1.2+). Direction (b1) is the right shape; Open Question 5's empirical-grounding constraint ("biased entities appear only if present in ≥N% of past sessions") is the bio-fidelity guard that prevents self-reinforcing preference loops at the fixture-scene-load surface.
...
5. **Self-reinforcing preference loop (bio-fidelity concern).** Phase 3 bio-fidelity review flagged that hooking NAc top-biases into the LLM-generated scene manifest closes the substrate-signal gap but risks a feedback loop: substrate prefers tool X → manifest generates entity for X → agent uses X → substrate reinforces X → next manifest is more X-biased. In biology, top-down priming is constrained by perceptual groundedness (V1's top-down signal *modulates* what's perceivable, doesn't *generate* new percepts). Maxim's manifest runs *before* the scene with no ground-truth correction. Candidate constraint: substrate-biased entities appear in the manifest only if they were present in ≥N% of past sessions (an empirical-grounding check the substrate has to earn). Audit this when Hookup 1 lands.
   ...
   **Fixture-path acuity (2026-05-28, Fix B pre-merge architecture-lens BLOCK 3):** OQ5 is structurally MORE acute on the fixture path than on the generative-narrator path. In the generative path, the narrator LLM is already a non-deterministic experimental variable — substrate-biased manifest entities are part of the LLM's output stream and the experiment treats LLM output as in-arm noise. In the **fixture** path (e.g., Roy's `roy_1_holdout.yaml`), the fixture YAML IS the experimental contract: pre-defined percepts, fixed expectations, the same scene every run by intent. Fix B (extending W2 to fixture scene-load) means Roy-3a Arm A (`substrate=from_priming`) and Arm C (`substrate=blank`) run against **structurally different effective scenes** — Arm A has substrate-favored entities materialized; Arm C doesn't. The Roy invariant "fixture is identical across arms" silently shifts from YAML-identity to YAML-identity-modulo-substrate. Wire-A's tau-300 decay is a *within-session* inhibitor, not the *cross-session* / *cross-arm* concern this raises. Fix B accepts the trade-off (the substrate→action gap closure is the 1.0 thesis bottleneck and the bio-mechanism is correct in shape — agents-with-prior-experience SHOULD see different effective scenes) but the methodology shift is named explicitly here so future Roy iterations + their post-hoc analysis treat YAML-identity as a weaker claim than effective-scene-identity. **Observability fold:** `FixtureResult.pretrigger_entities` now surfaces the materialized entity names per-arm so cross-arm scene-divergence is post-hoc auditable from `sim_log("SEM_TRACE", ...)` events + the per-arm result JSON. If pathological reinforcement appears in a future Roy iteration's cross-arm comparison, the empirical-grounding gate (≥N% past-sessions) becomes load-bearing for the fixture surface.
