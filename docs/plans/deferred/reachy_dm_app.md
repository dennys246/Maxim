...
> **⏸ DEFERRED 2026-08-27 — self-declared post-1.1 flagship track, not an MVP dependency; zero code here or in Maxim-pulse; no roadmap ownership. **Revive when** the Maxim-pulse MVP (setup + Talk + memory view) ships and FIT is closed on the Pi.**

**Status:** Shell plan, drafted 2026-07-23. Companion to [reachy_mini_app.md](../archive/reachy_mini_app.md). Post-1.1 flagship track — *not* an MVP dependency.
...
- **[deferred/dungeon_master_extensions.md](dungeon_master_extensions.md)** = *content-depth follow-ons* (encounter library, adaptation engine, sub-sim isolation…), explicitly gated on "revive when real DM usage produces a signal." **Do not revive that list preemptively** — this front-end is what produces the signal that tells you which extension matters. (E.g. "users complain about encounter repetition" → encounter library; "the DM steamrolls players" → adaptation engine.)
...
- [reachy_mini_app.md](../archive/reachy_mini_app.md) — the MVP app + setup + memory view (this plan's prerequisite)
...
- [deferred/dungeon_master_extensions.md](dungeon_master_extensions.md) — demand-driven depth follow-ons
