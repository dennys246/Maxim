...
> **⏸ DEFERRED 2026-08-27 — MVP seams ✅ SHIPPED (#416/#419/#424/#425/#426/#427/#428; `pi` extra) in `src/maxim/console/`. **Revive** VOICE + CONTENT with `reachy_dm_app.md` (the post-1.1 Adventure track).**

**Status:** Shell plan, drafted 2026-07-23. The Maxim-repo counterpart to the app-repo docs [reachy_mini_app.md](../archive/reachy_mini_app.md) (MVP) + [reachy_dm_app.md](reachy_dm_app.md) (Adventure flagship). Those *reference* these seams; this plan *specifies and builds* them.
...
**Known limitation — talk's reward signal is degenerate (`[engineering]`, documented deliberately).** Talk's reliably-firing tool is `respond`, which always succeeds, so per the motor-credit invariant `record_outcome` books `respond → +1` into NAc on essentially every turn. That is the same tool-success-floor saturation `MAXIM_OPERANT_ONLY_CREDIT` exists to counter and that [deferred/credit_on_progress_not_execution.md](credit_on_progress_not_execution.md) names as the root cause. So the honest reading today: **"Adventure teaches Talk" rests on the *adventure* side of the ledger** — adventure writes rich outcomes, talk mostly reads them back (via the enrichment pipeline + thought gate, wired 2026-07-28). Talk itself teaches the substrate little beyond "responding works". Not a blocker for the MVP — the read path is what the product claim needs — but it must not be implied otherwise by `talk()`'s docstring. Revisit when credit-on-progress lands.
...
**Work:** 2 hand-authored seed campaigns (the floor; imagination extends), plus a documented moderation stance for improvised narration to families. Scope minimally; let real Adventure usage (per the deferred [dungeon_master_extensions.md](dungeon_master_extensions.md) revive gate) drive depth.
