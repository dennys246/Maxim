...
> **DEFERRED (2026-07-15 plans audit):** 1.0 MVP SHIPPED (PR #287 — `Tool.auto_fire` + `Tool.kind`, auto-fire dispatch, grayscale-visibility prompt annotation). The 1.1+ full plan (sensory_events.jsonl split, LRU tuning, NAc predicate-outcome typing) is gated on explicit authorization, and its motivating Roy-3a line has since bird's-eyed toward encoder replacement. **Revive when:** a substrate→action conversion experiment re-prioritizes tool visibility, or predictive-learning work needs the sensory-event separation / predicate-outcome typing. **Revive path drafted (2026-07-19):** [passive_sense_discovery.md](passive_sense_discovery.md) proposes an ambient discovery channel whose Phase 3 depends on this plan's sensory_events.jsonl split + NAc predicate-outcome typing; authorizing that plan revives this scope.
