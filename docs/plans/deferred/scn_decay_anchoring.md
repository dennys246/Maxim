...
5. **Does this surface a sleep/replay timing concern?** During sleep (memory consolidation), the agent loop's tick rate drops dramatically. Today, decay fires less during sleep (matches "no thinking, no decay" intuition). After SCN-tying, decay fires at the same wall-clock rate during sleep as during awake. Is that the right bio-fidelity behavior? Probably yes (extinction continues during sleep biologically), but worth checking against `../archive/memory_consolidation_practice.md` predictions before Phase 3.
