...
by construction (a sensor vector against a text percept), which is this plan's own problem statement.
That line was revived 2026-09-19 as a PARALLEL line gated on a paired-data audit over survival-world
runs; if the audit passes, this projection plan revives WITH it. The audit is not this plan's Stage 0
(different data source, different pairing rule) and does not satisfy it.
...
> **2026-09-18:** a survival-world paired-data audit (a sensor vector + a language percept per tick; [roadmap_1_4.md](../roadmap_1_4.md) §JEPA) is a DIFFERENT audit with its own script and pairing rule; it does not satisfy or replace this stage. The microduck pairing hypothesis below stands, unscheduled: the duck moved with "Shared perception" to a physical-trigger deferral.
