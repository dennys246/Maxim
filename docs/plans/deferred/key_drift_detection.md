...
> **DEFERRED (2026-07-15 plans audit):** Never implemented — no `peer/key_fingerprint.py`, no `/v1/admin/key-fingerprint`, no `check_key_drift`. The reactive 401-message UX fold from config_unification (`40acd77e`) reduced urgency. **Revive when:** stale-key confusion recurs in a context the reactive 401 hint doesn't cover (quiet-peer `maxim doctor`, background lane probes).
