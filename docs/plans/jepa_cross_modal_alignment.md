...
**Status:** **Stage 4b CANDIDATE — awaiting Roy-5b (Stage 3) outcome (authorized 2026-05-28 post-[exp 34](../experiments/34_wire_a_post_fix_a_b.md)).** Originally DRAFT (2026-05-14, after Roy-5a-substrate-on's dimension-mismatch finding). Exp 34's clean substrate→action measurement (Fix A + Fix B both validated end-to-end; manifest LLM materialized 'set of keys' instead of a food entity given `sense_food_source [strongly rewarding]`) is the strongest direct motivation yet for this plan — the substrate→manifest-LLM bridge has a semantic gap exactly where cross-modal encoder alignment lives. If Roy-5b (`roy_5_encoder_alignment_disambiguator.md` Stage 3) shows zero Hebbian-binding edges even under deliberately-scaffolded co-firing, this plan promotes from "Stage 4b candidate" to "1.2 implementation in flight." If Roy-5b PASSES, this plan stays parked at "ready if cross_modal_substrate_binding.md hits its own ceiling" and Stage 4a takes precedence.
