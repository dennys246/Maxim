...
**Status:** **Stage 4b PARKED at "candidate" with WEAKENED motivation by Roy-5b-confound-isolation (2026-05-29).** Roy-5b-confound-isolation ([docs/experiments/36_roy_5b_confound_isolation.md](../experiments/36_roy_5b_confound_isolation.md)) showed the Roy-2c recognition gap is closed by a small (`Δ=0.04`) tweak to `ECConfig.pattern_complete_threshold` (0.40 → 0.44) — no scaffold needed, no binding mechanism needed. This weakens the JEPA motivation's "the gap is structurally undefinable in cross-modal cosine space" framing: if a tiny threshold tuning closed the gap, the gap is more tunable than the plan assumed. **The dimensional fact remains correct** (`SensorEncoder` 384-dim vs `LinguisticEncoder` 768-dim — cross-modality cosine is mathematically undefined); the bio-defense for needing JEPA gets weaker as the gap-closure problem moves from "structural" to "tuning." This plan stays parked as Stage 4b candidate but no longer has a clear promotion trigger — the disambiguator plan's Stage 5 question ("what explains the threshold-driven gap closure mechanistically, and what 1.1+ direction does it motivate?") may produce a different direction entirely. **Promotion criteria revised:** instead of "Roy-5b FAIL → JEPA in flight," the new criterion is "the disambiguator plan's Stage 5 question identifies a behavioral or substrate-level problem the threshold-tuning approach cannot solve, AND that problem is structurally about cross-modal alignment in different-dimensional encoder spaces." Until that surfaces, JEPA stays in its current parked state as research direction for a problem that may not exist anymore. **Pre-Roy-5b-confound-isolation history:** Originally DRAFT (2026-05-14, after Roy-5a-substrate-on's dimension-mismatch finding). "Stage 4b candidate" 2026-05-28 post-[exp 34](../experiments/34_wire_a_post_fix_a_b.md) divergence-in-a-row authorization. Roy-5b (2026-05-28) Conditional PASS / Ambiguous: did not produce clean FAIL needed for 1.2 promotion. Roy-5b-confound-isolation (2026-05-29) showed Roy-5b's gap closure was threshold-driven not scaffold-driven, weakening JEPA's structural motivation.
...
| [`v1_refinement.md`](v1_refinement.md) | 1.0 plan. | **JEPA is explicitly NOT 1.0 work.** No section of v1_refinement should depend on JEPA. If a 1.0 task surfaces that needs JEPA, the task itself is mis-scoped for 1.0. |
...
The plan stays DRAFT until [`roy_5_encoder_alignment_disambiguator.md`](roy_5_encoder_alignment_disambiguator.md) Stage 3 (cradle redesign) ships. At that point, Stage 0 of this plan runs and the design proposes / declines further stages based on the data audit.
