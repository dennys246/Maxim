...
**Status:** **CANCELLED by Roy-4 (2026-05-13); CONDITIONALLY ELIGIBLE for resurrection by Roy-5b (2026-05-28). NOT yet authorized.** Roy-5b ([docs/experiments/35_roy_5b.md](../experiments/35_roy_5b.md)) produced a Conditional PASS / Ambiguous verdict: 1 matched priming↔test edge at default rule, but the modality_pair is **(drive, drive) intra-modal between two priming food clusters** — NOT the cross-modal binding this plan's bio-defense (superior colliculus vis+aud, sensorimotor M1) is built on. Cross-modal (drive, linguistic) edges form at sub-default thresholds (`min_cofire ∈ {1, 2, 3}`) but drop out at `min_cofire=5`. Two gates must pass before resurrection authorization: **(1) Confound-isolation re-run** — Roy-4 spec at HEAD with standard `infant_humanoid` body + `MAXIM_SUBSTRATE_PATH=1` isolates whether Roy-5b's 0% → 100% recognition gap closure is scaffold-driven (this plan's mechanism is rescued) or threshold-driven (PR #264 raised `pattern_complete_threshold` 0.40 → 0.44, which alone may explain the closure — in which case this plan's rationale collapses). **(2) User classification call** on whether the (drive, drive) intra-modal matched edge counts as evidence the *cross-modal* binding mechanism is rescued, given the disambiguator plan's pre-registration table row 3 explicitly hatches "structural shape not matching the binary diagnostic → Pause." **No Stage 2 implementation LOC begins until both gates pass.** If resurrection IS authorized: the naming-event scaffold (or equivalent deterministic drive→linguistic co-firing emitter) is a Phase 0 prerequisite — the standard cradle arc does NOT provide the co-firing pattern this mechanism requires. Default rule `min_cofire=5` may need to drop to ~3 if cross-modal `(drive, linguistic)` edges are the load-bearing edge type the consumer side wants. Originally written 2026-05-13.
