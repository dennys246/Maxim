...
| Cross-session behavioral delta from substrate-carried memory (under LLM-AUT) | Wire-A annotation + Wire-1 annotation + NAc reward bias → action selection | **PARTIAL — investigation gate** per [Exp 37](../experiments/37_cross_session_graduation.md), with scale-dependent evidence base. **Qwen14B fire (2026-06-06):** primary Δ = −0.06 SD (FAIL), 0/4 corroborating, 0/3 ablations shrink (2 overshoot past A). **Qwen32B fire (2026-06-08):** primary Δ = **+1.43 SD (PASS)**, **2/4 corroborating PASS** (affordance-preference safe-fraction +1.22 SD, tool-class diversity −1.14 SD), robustness PASS — substrate-transfer signal IS detectable at 32B scale. **But:** secondary criterion still 0/3 ablations shrink B's delta toward A (Wire-A suggestive at +0.56 SD, Wire-1/NAc-bias produce no change), AND Arm C confound triggered on Qwen32B (C ≈ B, not C ≈ A — the substrate effect generalizes across priors, not scenario-specific). **Combined 1.0 framing:** substrate carries cross-session memory at all scales (Exp 10 + both fires); the carried memory measurably shifts behavior at ≥32B scale but the specific bio-mechanism attribution (Wire-A / Wire-1 / NAc-bias) is NOT cleanly supported and the effect appears broader-grained than scenario-specific. The strong "substrate drives action selection via specific bio-mechanisms" claim is **pulled from 1.0 bio-framing**; substrate-driven action selection stays deferred to Exp 38 substrate-primary work post-1.0. **Re-run on:** encoder swap, prompt construction change, Wire-A/Wire-1/NAc-bias mechanism refactor — re-validation re-fires Exp 37 with the same N=5 design at the matching git_hash for both Qwen scales. **Regression guard:** [docs/experiments/37_cross_session_graduation.md](../experiments/37_cross_session_graduation.md) + [docs/experiments/data/37_results.jsonl](../experiments/data/37_results.jsonl) (Qwen14B) + [docs/experiments/data/37_results.md](../experiments/data/37_results.md) (Qwen14B) + [docs/experiments/data/37_results_qwen32b.jsonl](../experiments/data/37_results_qwen32b.jsonl) (Qwen32B) + [docs/experiments/data/37_results_qwen32b.md](../experiments/data/37_results_qwen32b.md) (Qwen32B) + analyzer at [scripts/analyze_exp37.py](../../scripts/analyze_exp37.py). |
