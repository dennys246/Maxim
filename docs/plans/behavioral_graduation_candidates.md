...
| Cross-session behavioral delta from substrate-carried memory (under LLM-AUT) | Wire-A annotation + Wire-1 annotation + NAc reward bias → action selection | **PARTIAL — investigation gate** per [Exp 37](../experiments/37_cross_session_graduation.md) 2026-06-06 Qwen14B fire: primary metric `positive_approach_engagement_fraction` Δ in SD units = −0.06 (threshold +1.0); 0/4 corroborating; 0/3 ablations shrink B's delta toward A (2 of 3 overshoot past A). The LLM-AUT's pretrained priors empirically dominate substrate signal on the cradle scenario at N=5. Strong "substrate drives action selection" claim is **pulled from 1.0 bio-framing**; substrate-driven action selection is deferred to Exp 38 substrate-primary work post-1.0. **Re-run on:** encoder swap, prompt construction change, Wire-A/Wire-1/NAc-bias mechanism refactor — re-validation re-fires Exp 37 with the same N=5 design at the matching git_hash. **Regression guard:** [docs/experiments/37_cross_session_graduation.md](../experiments/37_cross_session_graduation.md) + [docs/experiments/data/37_results.jsonl](../experiments/data/37_results.jsonl) + [docs/experiments/data/37_results.md](../experiments/data/37_results.md) + analyzer at [scripts/analyze_exp37.py](../../scripts/analyze_exp37.py). |
