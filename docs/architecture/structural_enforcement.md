...
| [biosystem_unification.md](../plans/biosystem_unification.md) | **Waves 1+2 SHIPPED, Wave 3 in review** (2026-04-17) | Same shape applied to PainBus, ReactionBus, MemoryHub, DefaultNetwork, bio_stack umbrella | Layer 5: `runtime/bio_stack.py::build_bio_stack(*, persistence_dir)` composes all four builders |
...
| [memory_hub_unification.md](../plans/memory_hub_unification.md) | **Builder + migration committed** (2026-04-16, Wave 2 of biosystem_unification) | "forgot to call `.connect()` after `MemoryHub()`" — two production sites (cli.py non-sim, AgentFactory NPC) constructed `MemoryHub()` and never called `.connect()`, leaving PlanHistoryBridge + EscalationLearningBridge + FearCircuitBridge permanently `None`. A third site (orchestrator orch) was dead code (TypeError from missing `scn`/`ec` swallowed by `except Exception`). | Layer 5: `integration/memory_hub.py::build_memory_hub(*, hippocampus, scn, nac, ec, ...)` always calls `.connect()` internally. Five production sites migrated. Raw `MemoryHub()` intentionally still allowed for ~16 test sites. |
