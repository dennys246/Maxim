...
### Post-1.0 substrate, embodiment, and heartbeat experiments (38–62, R1–R3)
...
| [56_four_arm_sharing.md](56_four_arm_sharing.md) | 2026-09-06 | **complete — EARNED (graduate); the 1.2 "Oasis" headline** | Four-arm sharing benchmark: agent A's teacher-taught, world-keyed operant bias, exported as a signed bundle and ingested through the shipped 1.2 path (`maxim substrate ingest` — the V1–V10 adapter + `substrate_merge`, NOT bare `nac_merge`), changes independent agent B's FIRST-contact choice. Live Paper 1.16.5 world (not the mock), n = 50 receivers/arm, substrate-primary, at `main`-reachable `9905d4d8`. Taught raw **0.84** [0.71, 0.92] / bias-decisive **0.80** [0.67, 0.89] vs isolated floor 0.22 raw / 0.00 decisive; merged-satiated (zero-credit donor) 0.12 and dangling-half falsifier 0.12 both at floor. All four gates PASS — TRANSFERRED 0.80 ≥ 0.70, ABOVE-FLOOR 0.62 ≥ 0.20, WANT-NOT-FILE 0.72 ≥ 0.20, BOTH-HALVES −0.10 < 0.10 — plus anti-vacuity `kit_pass: true`. 40 of 50 taught first contacts decided by the situation-keyed learned-bias channel. Scope: one campaign, one world layout, teacher-taught wants — NOT scaling (Exp 57), hardware, aversion, or cross-layout (R1). |
| [protocols/exp56_four_arm_sharing_preregistration.md](protocols/exp56_four_arm_sharing_preregistration.md) | 2026-09-06 | pre-registration (amendments 1–4 all pre-confirmatory-data), merged `3f9ce733` (#639) | The four-arm design frozen before any confirmatory run: isolated / merged-taught / merged-satiated / dangling-half, one seed-paired donor per receiver, ε-greedy probe over a frozen roster, verdict constants and the anti-vacuity no-op kit fixed in advance. Amendment 2 moved the contingency slots far+high after the live shakedown measured the pre-amendment apparatus situation-blind. |
| [57_dose_response_ladder.md](57_dose_response_ladder.md) | 2026-09-08 | **complete — PARTIAL (pre-registered branch); the 1.2 scaling claim** | Dose–response ladder: pooling N independent PARTIAL learners. **MONOTONICITY (primary) PASS in its robust form** — per-agent trials-to-criterion τ **21 / 21 / 15.5 / 10.5** across N = 1/2/4/8, Jonckheere–Terpstra permutation p ≈ 1e-4, surviving the drop-fully-censored guard, with endpoint coverage widening 0.25 → 0.50 → 0.75 → 0.75 (so the win is coverage, not louder wants — the fold is convex). **NOT-JUST-MORE-DATA FAIL** — N × τ = **42 / 62 / 84** vs `single_matched` **41 / 46 / 43**: the pool spends MORE total experience than one agent given all of it, the convex-combination merge cost, named as the 1.3 audit target. Per-participant win, not a total-sample free lunch. NOISE-FLOOR / L2_SEED_VARIANCE / ANTI-VACUITY PASS. Live Minecraft, 4 rungs × 3 conditions × 20 cohorts (9,200 rows), at `main`-reachable `d6f12b1f`. Owner call at the release checkpoint: SHIP as a named PARTIAL. Not a scaling LAW; the failing gate is the strict serial sample-cost comparison, not wall-clock. |
| [protocols/exp57_dose_response_ladder_preregistration.md](protocols/exp57_dose_response_ladder_preregistration.md) | 2026-09-08 | pre-registration (amendment 1 pre-data; 2–3 post-data), merged `d15ca4c0` (#649) | GATE constants — rungs {1,2,4,8}, δ_eff = 0, p < 0.05, ≥ 20 cohorts — frozen at the prereg merge, NOT from Phase-0 data, with MONOTONICITY-passes-while-NOT-JUST-MORE-DATA-fails written in advance as the explicit PARTIAL branch. |
| [r1_cross_layout.md](r1_cross_layout.md) | 2026-09-07 | **CACHE-CONFIRMED (structural null)** | Does Exp 56's shared want carry a generalizable representation or a cached association? A cache: the want is an exact-key cluster entry, so a layout distinct enough to be a genuinely different situation necessarily misses the key — knowable from the substrate mechanism, so a live campaign is the wrong instrument. Bounds the 1.2 headline (transfer demonstrated, generalization not) and scopes the 1.3 readout work. A null ships as a null. |
| [r2_drive_premise_check.md](r2_drive_premise_check.md) | 2026-09-07 | **PREMISE-NULL** | Do the `minecraft_player` world-owned drives (health homeostatic, food entropic, both `drift_rate: 0.0` — the game drains them, not the model) measurably move behaviour toward corrective affordances? They do not; the survival loop has three specific breaks. Per the rung's own stop rule R3/R4 do not run in 1.2, and the three breaks become the 1.3 build list rather than a benchmark back-fitted onto a premise that does not hold. A null ships as a null. |
| [exp60_drowning_avoidance_prereg.md](exp60_drowning_avoidance_prereg.md) | 2026-09-16 | **complete — EARNED (graduate); the Wire-4 situation-fear's behavioural graduation** | Learned, anticipatory drowning-avoidance from GAME-NATIVE pain, no LLM in the action path: an agent held underwater until air-hunger pain, then rescued, leaves the water BEFORE the pain on later submersions; its yoked twin (same pain, same episodes, fear subscriber detached) never does. Live Paper 1.20.4 water classroom, 5 seeds/arm, 6 pain-free placements per probe. All five frozen gates PASS: FEAR post P(surface) **1.0** vs ABLATED **0.0** on every seed (ABLATED made zero executor calls), exact permutation **p = 1/252** (the floor for 5 v 5), specificity water fear −1.0 vs shore 0.0. Latency to air median 1.72 s inside the 4.34 s pre-pain window. **Caveat recorded:** after a seed's first escape the action carries a positive causal link, so the fear-only read is the FIRST post placement per seed (5/5 surfaced, 2.9–3.3 s). Planned as "dark = danger" — BLOCKED at the instrument (Exp 58), cue swapped to water. |
| [exp61_shared_fear_prereg.md](exp61_shared_fear_prereg.md) | 2026-09-17 | **complete — EARNED (graduate); the 1.3 "Oasis-2" headline** | A learned drowning-fear transfers between independent agents through the shipped signed-bundle path, discounted ×0.75 at ingest, and drives the receiver out of the water on its FIRST loop-live submersion (the lifecycle first submerges it once with the loop OFF at a representation gate). One campaign, one code hash, 121 rows, zero refusals: **12/12** transferred vs **0/24** isolated, **0/12** cluster-not-fear (cluster and percept valence shipped, fear did not) and **0/24** dangling (fear dropped at ingest for want of its world node); Fisher one-sided **p = 8.0 × 10⁻¹⁰**; six frozen gates PASS; the loop-OFF gate read the discounted fear (−0.75) exactly on every receiver. NOT earned: extinction, scaling past one donor, a received fear's reach to another pool, hive-side promotion. |
| [r3_survival_benchmark_prereg.md](r3_survival_benchmark_prereg.md) | 2026-09-18 | **instrument + frozen baseline — nothing graduated; frozen report INCOMPLETE, amended report COMPLETE** | One unrescued submersion per fresh agent on a depth-calibrated frozen gauntlet, 5 arms × 12, median time to air: A innate reflex **28.0 s**, B learns in situ **8.6 s**, C carries a self-learned fear **3.2 s**, D carries a received fear **3.1 s**, E same exposure as C with the subscriber detached **28.1 s**. Survival is a declared CEILING (12/12 everywhere, regeneration on): the drive buys ≈ 25 s of latency, ≈ 11 hp and ≈ 22 s of oxygen pain — **not life**. E ≡ A (exposure without the subscriber buys nothing); D ≡ C (p 0.84). The frozen report read INCOMPLETE on two INSTRUMENT rules (an unsatisfiable code-hash rule; a tick-period band that measured phase, not cadence, on few-second events); both amended post-data, instrument-only, two-lens reviewed, published BESIDE the frozen report. The frozen band had refused C's fastest rows, so restoring them moved C's median 0.10 s in the claim's direction — the amendment rests on the instrument argument, not the rows. |
| [exp62_pressure_interoception_prereg.md](exp62_pressure_interoception_prereg.md) | 2026-09-20 | **rung A — EARNED (graduate); rung B designed, not built** | Cross-context transfer BY THE BODY: the shipped `minecraft_player` body carries a learned drowning-fear from pool 1 to pool 2 (different altitude and spawn distance, same sealed-shell class, same frozen day) — no mechanism change, no new sensor, NO ingest. 27 rows, zero refusals, one code hash: cross **12/12** (Wilson lower bound **0.758** — the interval is the number, not 100 %), same **12/12**, fear-ablated **0/3** (zero executor calls; resolves to the SAME node with fear 0, so the contrast is the fear alone); five frozen gates PASS; Fisher one-sided cross v ablated **p = 0.0022**. **Not a general generalization result:** one discriminating world sensor (`is_in_water`, binary) — a two-point situation space; what carried is invariance to place, not to a changed situation. The context wall was measured: a night pool reads **0.799** against the 0.85 threshold, so the fear **misses at night** (rung B owns it). The agent's OWN fear — a received fear's reach to another pool is untested. |
