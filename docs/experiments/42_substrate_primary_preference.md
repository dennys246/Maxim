...
**Why gating is a HARD gate, not a re-weighting (front-gate justification, review fold).** The competing failure mode is an always-succeeding zero-stakes tool snowballing its causal confidence to the cap (~2.0); the drive-affinity nudge (≤ drive_value, ~0.5–0.9) is additive and structurally *cannot* out-add a maxed causal score, so raising the affinity weight wouldn't reliably win. The clean alternatives — capping/damping zero-stakes causal saturation, or EC-grounded relevance — are larger substrate changes; the hard gate is the minimal mechanism that isolates the question for this experiment (it rides on the existing `_DRIVE_TOOL_AFFINITIES` relevance set rather than introducing a new relevance notion). Recorded so the choice is on the record per the front-gate-scope principle.

**What the ablation does and doesn't isolate (review fold).** Gating-OFF demonstrates gating is *necessary*; it does not by itself isolate the *substrate's* contribution from gating, because within the gated set preference could in principle be driven by residual ordering/affinity rather than learned signal. The substrate-isolating evidence is **C1 (counterbalance identity-flip) + C2 (per-source link signs: harm net < safe net)** together — C1 proves the preference tracks the swapped safety contingency (not a fixed bias), C2 proves the learned substrate carries the safe/harm distinction. GRADUATE rests on H1 **and** the C1+C2 corroboration, not on gating + ordering alone. (C2 stays *reported* rather than a hard gate to preserve the frozen §4 metric; treat a GRADUATE with C2 failing as PARTIAL pending investigation.)
