...
- **[jepa_cross_modal_alignment.md](../plans/jepa_cross_modal_alignment.md) stays parked at "candidate"** but the motivating narrative weakens: if a 0.04 threshold tweak (0.40 → 0.44) closed the gap, the gap is more tunable than the JEPA plan's "different-dimensional cross-modal cosine is undefined" framing suggested. The dimensional fact is still correct; the bio-defense for needing JEPA gets weaker.
