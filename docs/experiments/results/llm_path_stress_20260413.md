...
2. **Phase D protocol ambiguity: recovery-time measurement.** Should specify "use a benchmark workload for recovery timing; use a sim for end-to-end robustness under agent workload". Worth a one-paragraph refresh in [docs/plans/archive/llm_path_fast_failover.md](../../plans/archive/llm_path_fast_failover.md) lines 405-470.
