...
| D20 | **The required fast suite is not offline/hermetic.** Installed optional ML dependencies can trigger model downloads; subprocess harnesses and teardown write under `~/.maxim`; CI runs only the narrower unit suite. | **OPEN — 1.1 gate.** Default fast tests must require no network, hardware, model cache, or writes outside their temporary root. The D22 verification pass isolated the cradle-mother subprocess workdir, but the suite still reproduced a Hugging Face download attempt and `CostTracker` home writes. | `tests/unit/test_clip_encoder.py`; `tests/substrate/test_p4_fixture_validation.py`; `models/language/cost_tracker.py`; `.github/workflows/test.yml`; investigation doc §D20 |
