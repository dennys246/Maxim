...
| D12 | **The orchestrator's narrator planning call can block UNBOUNDEDLY — observed 8,624s (2.4h) on one spinner with no timeout firing and the stall detector silent.** Live observation 2026-08-18 (big-mac-mini, `maxim --sim` generative, mistral-7b via auto-spawned server; server child alive but idle at 0:15 CPU — the request was lost/wedged, and no layer of the three-layer timeout anatomy gave up). Downstream symptom is deceptive: the AUT ticks sensors/drives normally while starving for percepts, which reads as "bio systems not engaging" when the bio stack is fine. Known-adjacent: `orchestrator.py::_stall_detector` carries the B2 limitation (hardcoded `lane_tier="large"`, reads `MAXIM_LANE_LARGE_TIMEOUT_S` env directly, not `LaneConfig`) and [deferred/stall_detector_timeout_awareness.md](../plans/deferred/stall_detector_timeout_awareness.md) Stage 2 never shipped. Operator mitigation until fixed: run walk sims with `MAXIM_HEARTBEAT=1` and treat a planning spinner > ~10 min as wedged (kill + relaunch; the #510-class locks make relaunch safe). | **OPEN** — trigger: next Sim-Short walk chapter run, or any narrator call observed > lane timeout. Fix direction: bound the orchestrator's planning call with the lane's resolved timeout + make the stall detector cover the planning phase. | Operator observation 2026-08-18 (spinner `(8624s)` + `ps etime 02:21:31`); `src/maxim/simulation/orchestrator.py::_stall_detector` (B2 note) |
