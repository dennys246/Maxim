...
| 2026-09-15 | **re-run of the 09-04 trace at HEAD after the `saturation` re-centre** (`minecraft_player` range `[0,10]`→`[0,20]`, Exp 60 gate (ii) finding; the analyzer re-reads body ranges at run time) | **same verdict, mitigation-confirmed**: A4 separation **0.0881**, stability **1.0**, discrimination **0.6852** (09-04: 0.0566 / 0.9984 / 0.6309 — those numbers stand as measured under `[0,10]`, pinned by the record's own provenance); monkeypatching ONLY saturation back to `[0,10]` reproduces the 09-04 numbers exactly, so the delta is that one sensor, moving favourably. A declared "rest" the world never rests at is a constant, not a neutral | `scripts/l11_real_trace_remeasure.py analyze` on the committed 09-04 trace; replay `docs/experiments/data/exp60_saturation_rest_check.py` |
