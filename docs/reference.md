...
| `src/maxim/simulation/roy_runner.py` | Roy R3: three-arm iteration orchestrator. Drives priming via `curriculum_runner`, runs the same held-out test scenario across arms (A = substrate-primed neutral, B = blank mode-labeled, C = blank neutral — labels only since the 1.1 persona hard-remove; nothing is prompt-injected), computes pairwise `substrate_diff`, persists `result.json` + `summary.md`. G3 LLM pre-flight probe aborts in <3s on unreachable leader (env or `peer.yml`). |
