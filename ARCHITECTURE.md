...
- `src/maxim/similarity/`: owns multi-modal similarity queries.
  - `ec.py`: Entorhinal Cortex - two distinct query surfaces, easily conflated. **Situation matching** (`find_similar` over `SituationSignature`) is LSH-STRUCTURED but **degenerate in production — it is a linear scan** (bugs ledger D51, measured 2026-08-30): `LSHIndex` buckets solely on `signature.semantic_hash`, which stays `(0,)*8` unless a `SemanticLSH` hasher is installed, and no production path installs one — so every item lands in ONE bucket. Its four hash tables are byte-identical for the same reason, giving 4x cost and zero probability amplification. Do not describe this surface as sublinear. **Substrate pattern routing** (`pattern_complete_or_separate`, the path the substrate results rest on) is an **exact same-modality centroid scan, O(Nd)** — no LSH, no approximation. Bio-mapping is FUNCTIONAL only: in the brain, pattern separation is a **dentate gyrus** function and pattern completion a **CA3** attractor function; entorhinal cortex is the interface, not the separator. The names here describe what the code decides, not a claimed isomorphism.
...
| **EC** (Entorhinal Cortex) | Medial temporal lobe (adjacent to hippocampus) | Similarity queries | **Both surfaces are linear in production.** `pattern_complete_or_separate` is an exact O(Nd) centroid scan by design; `find_similar` is LSH-structured but degenerate — one bucket, four identical tables (D51). Separation/completion are DG/CA3 functions biologically — FUNCTIONAL naming, not isomorphism |
