...
**[engineering] A PR showing all-green may have run NO tests.** A `CONFLICTING` PR cannot produce a merge commit, so the whole `pull_request`-triggered `Tests` workflow never fires — CodeQL runs off a different trigger and `gh pr checks` then renders three green rows that are indistinguishable from success (2026-08-30, PR #576: 10 commits including an `agent_loop.py` extraction, zero test evidence, page all-green; reopening does NOT fix it, only resolving the conflict and pushing does). **Before trusting a PR's checks, confirm the expected checks are PRESENT, not just that the visible ones are green** — and if the merge is BLOCKED while every required context is green, read `gh api repos/<o>/<r>/rulesets`: a RULESET requiring a CodeQL analysis is a SECOND surface, separate from classic branch protection and invisible to `gh pr checks`, and CodeQL's default setup needs a PUSH (a reopen will not do) — `gh pr view <N> --json mergeable -q .mergeable` must say `MERGEABLE`, and the list must CONTAIN `unit-tests` + `lint` + `release-build`. An absent check reads as a passing one. Same family as the vacuous-guard findings: a mechanism that does not run looks exactly like one that ran and found nothing. Full history: [docs/lessons/green-pr-with-no-tests-run.md](docs/lessons/green-pr-with-no-tests-run.md). Regression guard: process invariant — no automated test; the mechanically checkable form (assert the required job names appear in the PR's check list) is tracked follow-up work.

**[engineering] A fix ships with a CALLER, or it has not shipped — and a red gate that does not flip is data.** D43 (#590) shipped three correct, tested functions and left their COMPOSITION to call sites, of which there were zero: every consumer still called the old path, so the defect stayed live behind 9,820 green tests, clean lints and a clean architecture audit. Its PR quoted a real "0.0 → 1.0" measured on a sequence composed BY HAND in the verification script — a measurement of a possibility, presented as proof it happened. Caught only because the D44 gate was `xfail(strict=True)` and did NOT flip to XPASS after the merge; a non-strict marker stays quietly yellow forever. **Rules:** before declaring a defect fixed, grep the new symbols across `src/` + `scripts/` EXCLUDING tests — zero non-test callers means capability, not fix; say where a number was measured; write red gates `strict=True`; when one fails to flip, do NOT remove the marker or re-point it at the hand-composed sequence (that turns a ship gate into a test of a recipe). **A defect that lives in a COMPOSITION cannot be fixed by shipping better pieces** — when the bug is in the seam, the fix belongs in the seam, as one callable thing. Same family as the green-PR finding, one level down: a mechanism that does not run looks exactly like one that ran and found nothing. Full history: [docs/lessons/shipped-the-pieces-not-the-composition.md](docs/lessons/shipped-the-pieces-not-the-composition.md). Regression guard: [tests/unit/test_d44_merge_behavioural_delta.py](tests/unit/test_d44_merge_behavioural_delta.py) — the strict-red-gate pattern plus its own anti-vacuity arms; the caller-grep half is process.
...
**Policy (decided 2026-08-29, roadmap 1.1.x item 16.1): `main` is ahead of PyPI; the bump happens in the release transaction.** Between releases `pyproject.toml` and `src/maxim/__init__.py` carry the LAST PUBLISHED version and `CHANGELOG.md` accumulates under `## [Unreleased]`; the bump commit (release PR) moves that section under `## [X.Y.Z] - <date>`, then build → publish → tag on the published commit → GitHub Release, per [docs/publication_guide.md](docs/publication_guide.md). The three living sync lines — in this file, `docs/plans/README.md` and `docs/index.md` — no longer assert what PyPI serves: they name the pyproject version and link https://pypi.org/project/pymaxim/ for the served one (dated CHANGELOG sections and release announcements stay historical records). A `src/` change still obliges a release cut eventually — the trigger is unchanged (runtime behavior, CLI interface, peer/leader protocol) — but it does NOT bump on the spot; **that half is convention, not yet enforced**: the card's Release-governance condition (4), a diff-scoped check that a post-tag `src/` commit adds an `[Unreleased]` line, is owed (roadmap 16.10). Enforced by [scripts/lint_version_sync.py](scripts/lint_version_sync.py) in CI: pyproject == `__init__` == a `## [version]` CHANGELOG header == the three version lines, which may carry no "pending"/"rc"/"serves" prose. Check locally: `python -c "from maxim import get_version_info; print(get_version_info())"` or `maxim peer version` (mismatch → the leader needs `maxim peer update && maxim peer restart`).
...
Current version: **1.1.2** (`pyproject.toml` + `src/maxim/__init__.py`; PyPI: https://pypi.org/project/pymaxim/ — `main` is ahead of PyPI by policy, §Versioning). 1.1.2 is the **"Decomposition"** release: fail-loud **Stage 2** measured (zero firings across 75,654 records — Stage 3 is green-lit), the first `run_agentic_loop` extraction (3,546 → 3,512, sections 7 and 8.5 to module-level functions), and four gates that could not previously fail — a `release-build` job (nothing in CI built a wheel before: bugs ledger D47/D48), the D12 guard failing by assertion rather than by hanging, a nightly `-m slow` lane (41 marked tests had been running nowhere), and roadmap item 16.10. 1.1.1 was **"Enforcement"** (items 16.1–16.9). The 1.1 "Sensorimotor" line: items 17 (Exp 52) and 19 (Exp 53b) DONE; item 18 → 1.1.3. This line is checked by `scripts/lint_version_sync.py` — bump it in the release transaction. Active theme **1.1 "Sensorimotor"**. The roadmap index is [docs/plans/README.md](docs/plans/README.md); the roadmap through 1.3 is [docs/plans/roadmap_1_1_to_1_3.md](docs/plans/roadmap_1_1_to_1_3.md); behavioral-graduation gates live in [docs/plans/behavioral_graduation_candidates.md](docs/plans/behavioral_graduation_candidates.md). Deferred plans (revive on trigger): [docs/plans/deferred/](docs/plans/deferred/). Shipped-history through 2026-04 (the old "Recently shipped" ledger): [docs/lessons/active-initiatives-history-2026.md](docs/lessons/active-initiatives-history-2026.md).
