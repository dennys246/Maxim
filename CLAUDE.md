...
Current version: **1.0.9** (`pyproject.toml` + `src/maxim/__init__.py`), **published to PyPI 2026-08-23** (tag `v1.0.9` at `5cb4413b`; the first publish since 1.0.0 — PyPI now serves 1.0.9). Everything merged after that tag is unreleased and lands in the next version, which must be > 1.0.9. This line is in the release procedure's sync table — update it with the version bump, not after. Active theme **1.1 "Sensorimotor"**. The roadmap index is [docs/plans/README.md](docs/plans/README.md); the roadmap through 1.3 is [docs/plans/roadmap_1_1_to_1_3.md](docs/plans/roadmap_1_1_to_1_3.md); behavioral-graduation gates live in [docs/plans/behavioral_graduation_candidates.md](docs/plans/behavioral_graduation_candidates.md). Deferred plans (revive on trigger): [docs/plans/deferred/](docs/plans/deferred/). Shipped-history through 2026-04 (the old "Recently shipped" ledger): [docs/lessons/active-initiatives-history-2026.md](docs/lessons/active-initiatives-history-2026.md).
