...
- **[engineering] Persistence uses `maxim.utils.atomic_io.atomic_write_json`** (fsync + tmp cleanup). Don't hand-roll `open().write()` + `os.replace()`. Regression guard: [src/maxim/utils/atomic_io.py](src/maxim/utils/atomic_io.py) is the canonical writer; [scripts/lint_atomic_io_ratchet.py](scripts/lint_atomic_io_ratchet.py) in CI (lint job, "atomic_io ratchet" step, since 2026-08-29) prints the per-file AST count of hand-rolled `os.replace()` call sites every run and fails any branch that raises a file's count — the current count is whatever that step printed on the latest `main` run, not a number kept here (the 2026-08-13/19 note said 17 from a text grep that also counted comments; the ratchet started at the AST truth). Burn-down to zero is its own task; the ratchet guarantees it only goes down.
