...
- **[engineering] No NEW silent exception swallows** — never add a bare `except Exception: pass`; narrow the exception type, or handle-and-log. Existing sites (~430) predate the rule and are grandfathered per-file. The deleted `@resilient` decorator must not be cited or re-introduced. Regression guard: `scripts/lint_no_silent_swallows.py` (CI step "No-silent-swallows lint") — per-file swallow counts grandfathered at origin/main; a branch may not increase any file's count.
...
**Run narrow first, then wide.** Test the specific module you changed before the full suite (~3 min, several thousand tests). **Kill stale sims before running tests** (`pkill -f "maxim.*sim" 2>/dev/null; sleep 2`) — a running sim holds GPU + port resources and causes hangs. **Threading pitfalls:** use `threading.RLock` (not `Lock`) if a method acquires the lock then calls another method that also acquires it — regular `Lock` deadlocks on re-entry; thread-safety tests that appear to hang are usually deadlocked, not slow. **Don't run sims from tests** — sims call real LLMs; tests mock them. Peer/tunnel checks: `curl -si -H "Authorization: Bearer $KEY" https://maxim.yourdomain.com/v1/models`; guides in [docs/troubleshooting/](docs/troubleshooting/).
...
Current version: **v1.0.6** on PyPI as `pymaxim`; active theme **1.1 "Sensorimotor"**. The roadmap index is [docs/plans/README.md](docs/plans/README.md); the roadmap through 1.3 is [docs/plans/roadmap_1_1_to_1_3.md](docs/plans/roadmap_1_1_to_1_3.md); behavioral-graduation gates live in [docs/plans/behavioral_graduation_candidates.md](docs/plans/behavioral_graduation_candidates.md). Deferred plans (revive on trigger): [docs/plans/deferred/](docs/plans/deferred/). Shipped-history through 2026-04 (the old "Recently shipped" ledger): [docs/lessons/active-initiatives-history-2026.md](docs/lessons/active-initiatives-history-2026.md).
