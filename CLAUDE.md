...
- **Drive protocol: `HomeostaticDriveSpec` and `EntropicDriveSpec` are compartmentalized types** (cradle Stage 1a, 2026-04-26). Homeostatic drives (temperature, pressure, stamina) drift toward `set_point`, pain proportional to deviation beyond `comfort_band`. Entropic drives (hunger, thirst, fatigue) drift away from equilibrium, pain at `deprivation_threshold`. Both live in `embodiment/sem.py`. `CouplingSpec` and `ModulationSpec` are 1.0 interface reservations (parsed from YAML, not evaluated). `pain_model: str` field future-proofs the homeostatic pain formula. Do NOT add fields to these frozen dataclasses post-1.0 without a breaking-change plan.
- **Entity acquisition via `entity_acquired`/`entity_released` side_effects** (cradle Stage 2, Mechanism B from proprioceptive_discovery.md). When the agent calls `pick_up` on an acquirable entity (`metadata["acquirable"] == True`), `ModulatorAffordanceTool` emits `entity_acquired` in `ToolOutput.side_effects`. `Executor._handle_entity_acquisition` reparents the entity to the agent's body, registers its tools, and flips ownership in `EntityMap`. `drop` reverses via `entity_released`. Acquired entity sensors contribute to the damage model while equipped.
- **`self_effect` on `AffordanceSchema` writes to agent body sensors** (cradle Stage 3). Scene entity affordances declare `self_effect: {sensor: delta}` in YAML. Fires only on voluntary agent actions (not reflex/orchestrator). Supports qualified modulator sub-sensor names (`arms.thermal`). Clamped to sensor range. Logged via `sim_sensor()`.
- **Three interaction levels for entities:** Observe (look/examine — no contact), Touch (self_effect — brief one-time sensor delta), Pick up (entity acquisition — sustained contact, sensors join body). The three-layer sensation model: Contact (Layer 1, entity acquisition), Proximity (Layer 2, orchestrator sensor writes), Narrative (Layer 3, keyword reflex fallback). All converge on: sensor change → `evaluate_failures()` → PainBus → NAc.
- **`NarrativePhase.act` and `.world_entities` structure long-horizon sims** (cradle Stage 5). `act: str | None` groups phases into narrative acts. `world_entities: tuple[str, ...]` lists component refs activated on phase entry. The generative runner activates entities per-phase with imagination fallback for missing refs. Entities persist across phases (accumulate, not reset).
- **`EnergyReactionBridge` and `MovementEnergyTracker` are DELETED** (cradle Stage 1c). Zero callers — hard-removed as dead code. Interoceptive drive signals are handled by the drive protocol. `LLMEnergyTracker` and `EnergyRegistry` remain live.
- **Embodiment tick cycle lives in `percepts.py`, not `body.py`.** `EmbodimentPerceptSource.next_percept()` runs: evaluate_failures → tick_vital_drift(dt) → format_body_state_for_prompt. Wall-clock dt (`now - last_poll`, not planned interval) ensures drives advance during LLM latency.
...
| Embodiment | `embodiment/sem.py` (Entity, Sensor, Modulator, AffordanceSchema with `requires` + `self_effect`, **HomeostaticDriveSpec**, **EntropicDriveSpec**, CouplingSpec, ModulationSpec), `embodiment/body.py` (evaluate_failures, derived health, component integrity, **drive pain evaluation**, **_publish_drive_pain**), `embodiment/spec.py` (SpecModulator, _parse_drive_spec), `embodiment/percepts.py` (EmbodimentPerceptSource — tick cycle, wall-clock dt), `embodiment/entity_map.py` (name→Entity resolution, **transfer_to_self/transfer_to_scene**), `embodiment/tool_bridge.py` (entity tool generation, affordance `requires` gating, **self_effect write-back**, **entity_acquired/entity_released side_effects**), `embodiment/resolution.py` (LOD depth), `embodiment/cerebellum.py` (forward models), `embodiment/motor.py` |
