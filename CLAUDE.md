...
**Three-times-is-structural:** When the same bug shape ("forgot to call helper X on path Y") gets fixed in three different locations, the helper is the wrong layer. Push the invariant DOWN into the type/constructor signature so the failure mode becomes a `TypeError`, not a silent no-op. The `build_executor(pain_bus=...)` keyword-only requirement is the canonical example: three identical bugs in `sem_execution_hook.md` Stages 1+2+2c (the bridge was missing on three different agent entry points) converged on `runtime/embodiment_bootstrap.py` as a shared helper, but call sites still had to *remember* to call it. The fourth instance never happened because `executor_bootstrap_unification.md` made forgetting impossible at the constructor level. Rule: count repeated bug-shape occurrences before writing the next helper. Two instances → fix locally. Three → push the invariant down a layer.
