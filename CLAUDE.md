...
Current version: **1.1.0rc1** (`pyproject.toml` + `src/maxim/__init__.py`) — the 1.1 "Sensorimotor" release candidate; **PyPI serves 1.1.0rc1 as a pre-release** (published 2026-08-25, tag `v1.1.0rc1` at `eae6559c`; `pip install --pre pymaxim`) — plain `pip install pymaxim` still resolves to **1.0.9** (2026-08-23, `v1.0.9`). `1.1.0` final is gated on roadmap step 5b only (website audit + cut; item 17 DONE 2026-08-25, item 18 deferred to 1.1.1). This line is in the release procedure's sync table — update it with the version bump, not after. Active theme **1.1 "Sensorimotor"**. The roadmap index is [docs/plans/README.md](docs/plans/README.md); the roadmap through 1.3 is [docs/plans/roadmap_1_1_to_1_3.md](docs/plans/roadmap_1_1_to_1_3.md); behavioral-graduation gates live in [docs/plans/behavioral_graduation_candidates.md](docs/plans/behavioral_graduation_candidates.md). Deferred plans (revive on trigger): [docs/plans/deferred/](docs/plans/deferred/). Shipped-history through 2026-04 (the old "Recently shipped" ledger): [docs/lessons/active-initiatives-history-2026.md](docs/lessons/active-initiatives-history-2026.md).
