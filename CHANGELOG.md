...
- **Every new memory now records how strongly it encoded (memory-strength Phase 2c-2).** Two fields
  ...
  `n / (n + 50)` over the store's size at capture, so an empty store, which calls everything novel,
  does not encode its first traces at maximum strength; drive pressure is **relevance-gated and
  fails closed**, counting only for drives the action actually relieved, so a starving stretch tags
  what touched hunger rather than everything that happened while hungry.
...
GAME — its own pain — and shares a survival want. On a live Paper 1.20.4 world, with no LLM in the action path: an agent
...
its FIRST loop-live submersion (**Exp 61 EARNED — the 1.3 headline**); and the **R3 survival benchmark**
...
### Re-baseline
- **Exp 56 re-baselined on Paper 1.20.4 (RB-1) — PASS, identical to the EARNED 1.16.5 campaign (a same-seed reproduction, not an independent replication).** The world builder ported to 1.20.4 with a 128-layer flat stack (the 1.18+ world floor is y=−64; `verify` now MEASURES the surface at y=63), shared Paper-server helpers for both world builders (`scripts/_paper_server.py`), and the server's own `version` reply stamped on every row (the campaign refuses another platform, the analyzer refuses a mixed file). Phase 0 5/5 with every reading identical; taught 0.84 raw / 0.80 decisive, isolated 0.22, satiated 0.12, dangling 0.12, `kit_pass: true`. One duplicated row from an operator restart is disclosed and moves no gate; a restart without `--resume` onto a non-empty `--out` now refuses. Exp 61's reuse of the fabric on 1.20.4 is covered.
### Added
- **Exp 61 EARNED — shared survival fear: a learned drowning-fear transfers between independent agents through the shipped signed-bundle path and drives the receiver's FIRST loop-live submersion (the 1.3 Phase-2 headline).** One campaign on big-mac-mini at one code hash (`4e25b475`, 1.10 h, 121 rows, zero refusals): 12/12 transferred receivers executed `escape_water` on their first loop-live submersion (escape call median 1.72 s, first air median 3.14 s, all inside the US-free window) against 0/24 isolated, 0/12 cluster-not-fear (the ablated donor's cluster and percept valence shipped, the fear did not) and 0/24 dangling (the fear dropped at ingest for want of its world node); Fisher one-sided p = 8.0e-10 and 3.7e-7; the loop-OFF gate read the discounted fear (−0.75) exactly on every receiver; the anti-vacuity kit passed. Prereg `docs/experiments/exp61_shared_fear_prereg.md` §Outcome; data PR #748; ledger Tier 1.
- **R3 harness (1.3 Phase 3, build step 3 — the lethal-window survival benchmark; prereg v3.1, twice four-lens reviewed).** `WaterTrial.lethal_event`: ONE unrescued submersion per fresh agent with the pilot's must-nots fixed (shore teleport BEFORE the loop stops on a surface, no linger; death from the `deaths` objective per sample corroborated by the respawn discontinuity across the scoreboard-leads-respawn race; the cap a Refusal carrying the row; one wall clock; 4 Hz held with `state_age_s`; decision provenance from `RecommendCapture`; per-drive pain-seconds integrated from the SAMPLE series with `drive_pain_for_value`; the before-damage flag from the event's OWN onset; a refused submerge STOPS the loop thread it started; captured events filtered to the row's agent). Preflights: `deaths()` now RAISES on a parse failure (Exp 60/61 callers sit under the refusal handler — a parse failure refuses a seed instead of silently reading 0), `preflight_deaths_objective` (exists / set 0 / read back), `read_food_state` (the TRUE saturation reservoir behind the bridge clamp — `docs/wiring/sensor-range-clamps.md`), `check_surface_cell_air`, `check_flee_anchor` (refuses NoPath / no anchor / > 0.5 s), `set_gamerule` read-back, `R3_GAMERULES` = the frozen five + `naturalRegeneration true`, `drowningDamage true` (the real 1.20.4 name), `doInsomnia false` (also in the builder's gamerules). `r3_run.py`: `cal` (the one verification cell: the innate-only floor arm ×12 at depth 5 → `r3_gauntlet.json`: reservoir band, tick-period band, roster, flee reply, cal hash, rows' sha, the floor's distributions — validated on write), `bench` (five arms interleaved by seed at one hash; refuses a gauntlet that does not validate or whose cal hash is not on `origin/main`; per-row drift refusal; settle-guard breach before the DV refuses, after it marks dirty; a stale sample refuses in cal and bench alike; Exp 61's campaign built EAGERLY for arm D), `report` (pure: per-arm distributions with Wilson/bootstrap intervals, contrasts named by arm with the mechanism beside each, Mann–Whitney on time to air, INCOMPLETE causes incl. the anti-vacuity reads and, with `--gauntlet`, rows off the gauntlet's hash; nothing graduated). Arm D reuses Exp 61's donor and receiver seams through public names on `exp61_run` (`Exp61Campaign`, `build_aut`, `load_json`, `rss_mb`, `world_ids`). `scripted_water.py`: a scripted drowning (`damage_onset_s`, `damage_per_s`, `respawn_lag_s`) with death, the objective leading the respawn snapshot, respawn at the shore with the game-native saturation 5, the apparatus heal honoured (a true reservoir 20 behind a sensed clamp 10), the RCON verbs the harness issues, teleports logged in order. Tests RUN offline before the rig: `tests/unit/test_water_trial_lethal.py` (surface / innate route at the game's rate / death across the race / cap / preflights) and `tests/unit/test_r3_run.py` (pure halves + an offline campaign — apparatus row and one event per in-process arm at the game's 16 s onset, the gauntlet written and drift judged). Two-lens code review folded (executor: the death race, the loop-thread leak; architecture: the three recorded-but-never-enforced guards, gauntlet validation, the docs).
- **R3 bench amendments (post-data, INSTRUMENT-ONLY; prereg §Amendments, two-lens floor review folded).** `r3_run.report --amended` beside the frozen report: Amendment 1 — the frozen "rows at a hash other than the gauntlet's" rule was unsatisfiable by construction (the cal data PR advances main); the enforced precondition is `harness_unchanged_between` (cal hash an ANCESTOR of the bench hash via `git merge-base --is-ancestor` AND no behaviour-bearing tracked file — `scripts/`, `src/`, `tests/`, `pyproject.toml`, `data/`, `scenarios/` — in the diff; fail-closed; the report carries a flag, the gauntlet's hash is never rewritten). Amendment 2 — the loop tick-period band (median ± 2 IQR of 46-period floor windows) collapsed to the `flee` tie-break interval on 1–2-period escape windows and refused seven C/D rows by tick phase; `reclassify_under_amendments` recounts a row whose ONLY refusal is that band, numeric and ABOVE it, event complete, with no later clean row for the same (arm, seed); `idle_tick_period_median_s` is the reported cadence covariate (0.555–0.583 s across arms; C vs A p = 0.76). Lesson filed: `docs/wiring/instrument-band-statistic-matches-window.md`. **R3-bench OUTCOME (prereg §Outcome):** frozen report INCOMPLETE on the two instrument rules; amended report COMPLETE — 60 events, five arms × 12 at one hash, survival a ceiling everywhere, A 28.0 / B 8.6 / C 3.2 / D 3.1 / E 28.1 s to air (C − A p 3.7e-5; E ≡ A: exposure without the subscriber buys nothing; D ≡ C: the discounted vicarious fear escapes like the self-learned one; C's 3.18 s = first tick 0.99 + tie-break 0.77 + ascent 1.43). An instrument and a frozen baseline; nothing graduated.
- **R3 offline campaign test → `slow` lane.** `test_offline_campaign_apparatus_and_one_event_per_in_process_arm` composes the whole campaign in one process (five agent loops + the scripted bridge + the client reader under one GIL); the frozen 0.15 s stale-sample rule is a real-time property of that process and a shared CI runner cannot hold it (0.348 s stall on the E event, PR #761; the rig refused E 388 at 0.169 s by the same mechanism). Neither the limit nor the refusal is relaxed — the test moves to the nightly slow lane and stays the local pre-PR check for every harness change.
- **Wire-4 fear travels in bundles (Exp 61 fear transport, 1.3 Phase 2 — the Exp 58 deferral discharged).** `cluster_fear` now ships through the signed-bundle path and lands on the receiver: the export scrub ships it clamped `[-1, 0]` and allowlisted; ingest bounds it, charset-checks the cluster id, REFUSES an out-of-allowlist failure mode (never strips) and multiplies admitted fear by `FOREIGN_FEAR_DISCOUNT` (0.75 — vicarious conditioning is real, drives avoidance without the observer's own US, and is weaker than direct conditioning; a re-export hands the next hop the discount squared; `merge-nac` trusted-local stays verbatim); `rekey_nac_state` re-keys it through the aligned EC id map with the agent id rewritten to the receiver's and drops fear whose cluster did not survive; `SubstrateMergeResult`, `IngestReport`, the ingestion journal and `maxim substrate ingest`'s print carry `fear_rekeyed` / `fear_dropped` / `fear_below_floor` (a fear at or below θ after the discount re-keys cleanly and never acts — counted); `prune_nac_cluster_biases` prunes fear with the biases; `nac_merge` min-folds as before. The allowlist and θ are one source (`decisions/nac.py::DEFAULT_CLUSTER_FEAR_FAILURE_MODES` / `DEFAULT_CLUSTER_FEAR_THRESHOLD`). Nine sites moved together per [docs/experiments/exp61_shared_fear_prereg.md](docs/experiments/exp61_shared_fear_prereg.md) §Mechanism; the composition gate `tests/unit/test_exp61_fear_transport.py` (landed RED `xfail(strict=True)` in #742) flipped in place; the Exp 58 posture pin `test_bundle_scrub_excludes_fear` flipped deliberately. Mechanism enters `[engineering]`; the behavioural claim waits for the Exp 61 run. **Harness (build step 3):** `scripts/survival_world/water_trial.py` lifts Exp 60's seed context (`WaterTrial`: rescue/submerge, every preflight, the loop window, placement, propose-only training, the live G2 read) out of `exp60_run._run`'s closures into one class both harnesses use — the Exp 60 verdict is byte-identical before and after; `exp61_run.py` (`run`: apparatus checks once per pair on a throwaway agent, donors trained inside a hub session and exported from STAGED files with sanity on those files, the six-step receiver lifecycle, the anti-vacuity kit as a campaign row, durable `--workdir` + `--resume`; `verdict`: Wilson intervals, Fisher's exact one-sided test on the receiver binaries, the six gates, drift and one-code-hash refusals); `scripted_water.py`, a deterministic water-classroom bridge + RCON fake, drives the offline smoke (`tests/unit/test_water_trial_smoke.py`) that proves the shared class ticks, acts through the bridge with zero executor calls, executes the escape under fear, and PERSISTS fear at the staging close — the loop's own session pair had closed the hub session, so without the re-open every donor would have staged fear 0 (two-lens review; the smoke is what would have caught it).
- **Encoder golden pin (`tests/unit/test_encoder_golden_v1.py` + `tests/fixtures/encoder_golden_v1.json`).** `_sensor_embed` underlies every substrate result in the graduation ledger and had no exact-output guard: the byte-identical test mirrored the formula in-process with `approx`, so a refactor that moved the bases or the accumulation order together with its expectations stayed green. The pin holds full 384-float vectors (exact equality) for seven readings × {gain None, 3.0} × {range-blind, -aware, -partial} and the LITERAL geometry-tag strings for the shipped spaces (world gained; interoception for the infant-operant and Reachy-infant bodies; audio), generated at `8889edaf`; the tag half runs two-process with differing `PYTHONHASHSEED`, the world tag is also pinned through the `encode_sensors` seam, and four anti-vacuity arms prove the pin sees a normalization, basis, set-point-style weight, or tag-field change. Regenerate only from the pre-change commit with a justified diff (`--regen`). H3 of the set-point-aware-neutral design review, landed BEFORE any encoder edit as that review required.
- **Body rest-at-neutral lint in CI (`scripts/lint_body_rest_neutral.py`).** The A4 range principle — every gained sensor's range is declared so its RESTING value is the midpoint — lived in YAML comments and broke once unseen until a live run (Exp 60 gate (ii) FAIL at cos 0.8502: `saturation` [0,10] while the game rests at 10). Every gained ranged sensor now declares its rest — an explicit `rest:` key (inert at runtime; the reading schema drops it) or, for a homeostatic drive, its `set_point` (never both — one rest source per sensor) — and the lint fails when it is missing or off the range midpoint; `rest: null` is the explicit no-rest declaration, PRINTED every run as the constant-mass roster and test-pinned (today: `light_level`, `time_of_day` ×3, one inherited). The lint walks every component category, resolves `extends:` chains with the registry's merge semantics (a child that re-declares only `range:` is linted against the rest it inherits — the Exp 60 shape one level down), defaults a homeostatic drive's missing `set_point` to 0.0 as the runtime does, and fails on an empty or missing tree. `initial:` is deliberately not read as a rest (it is a boot value, and it was the wrong field for saturation). The three Minecraft bodies declare `rest:` on their 22 drive-less gained sensors (health/oxygen rest via set_point). The lint checks the DECLARATION; the measured half stays with the apparatus check. H1 of the set-point-aware-neutral design review. Guard: `tests/unit/test_lint_body_rest_neutral.py` (also pins the lint's gained-modality set to `SensorEncoderConfig.gain_modalities`).
- **Exp 60 EARNED — learned, anticipatory drowning-avoidance: the Wire-4 situation-fear mechanism's behavioural graduation (1.3 Phase 1).** An agent that suffered air-hunger pain underwater leaves the water on later submersion a median 3.4 s BEFORE the pain would fire; its yoked twin with the same pain exposure but the fear subscriber detached never does. Live Paper 1.20.4 water classroom, FEAR vs ABLATED, 5 seeds/arm, 6 US-free placements per probe, production read→act path (PainBus `drive:oxygen` → `NAc.record_cluster_fear` → `anticipatory_threat_need` → `recommend_action` → `escape_water` through `run_agentic_loop`). All five frozen gates PASS: FEAR post P(surface) median 1.0 (from 0.0 pre, every seed), ABLATED 0.0 (zero executor calls), exact permutation p = 1/252, shore fear 0.0 vs water fear −1.0 on every seed. Caveat recorded: after a seed's first surface `escape_water` carries a positive causal link, so only the first post placement per seed (5/5 surfaced) is a fear-only read. Pre-registered ([docs/experiments/exp60_drowning_avoidance_prereg.md](docs/experiments/exp60_drowning_avoidance_prereg.md), frozen #729 before any trial; amendments 3–7 instrument-only), data PR #736, Tier-1 ledger row in [docs/plans/behavioral_graduation_candidates.md](docs/plans/behavioral_graduation_candidates.md). Run 1 was INCOMPLETE at the instrument (fear readable, zero actions) and drew the three runtime/harness fixes below (#730–#733). Not claimed: transfer between agents (Phase 2), other geometries, innate-vs-learned beyond the ablation.
...
### Fixed
- **`NAc.credit_node` stored a `0.0` key for every node a PAIN was credited to.** Negative credit clamps to zero by design (bias only widens recognition), but the write path kept the key, so a node the agent had only ever been hurt at carried a phantom `reward_bias` entry: read as 0.0, decayed away on the next prune, and shipped in every bundle meanwhile. Exp 61's dry run 1 (2026-09-17) refused both donors on three such keys. A bias that clamps to zero is now removed, the meaning the decay prune already gave it below 0.001. No persisted format change (fewer keys; an older file's zero keys still load and are pruned on the first decay tick). One behaviour change beyond fewer keys, and it is corrective: in a hivemind merge a PRESENT 0.0 key was a zero vote that halved another donor's positive bias on the same node, while an absent key is no evidence — the positive bias now survives. The signed goal/cluster bias surfaces store a pain as a real negative value and never shared the wart. Regression guard: `tests/unit/test_nac.py` (`test_negative_credit_on_a_fresh_node_stores_no_key` and siblings) + the Exp 61 smoke's staged footprint.
- **`run_minecraft_aut` ran its loop at the default PLANNING autonomy level, under which no body affordance is ever executed.** PLANNING answers "requires human approval for all actions" to every body affordance and the non-interactive prompt rule still reports interactive, so a learned fear proposed `flee` on every tick and the executor never saw a call (Exp 60 diagnostic, 2026-09-16; no harness on this path had ever executed a body affordance — the always-allowed no-op head tools were the only exception). `_loop_kwargs` now hands the loop an AUTONOMOUS controller, as the orchestrator does for its sim AUTs. Known, unchanged: the loop's PLANNING "non-interactive auto-approve" branch is dead code (`should_prompt("plan_approval")` is unconditionally True), and the consecutive-same-tool cap (5 identical params) now applies to executed substrate proposals too — both filed as follow-ups in the simulation brief. Regression guard: `tests/unit/test_substrate_primary_wake.py::test_substrate_proposal_is_executed_on_the_harness_loop` (RED on the pre-fix runner).
- **Substrate-primary agent loop idled forever on a bridge that emits no text events.** The idle gate woke only on pending input/work, a sim percept (the Minecraft percept source's EVENT queue: chat/death), a carried percept, the first step, or an awaited LLM (the Minecraft harness path passes no LLM worker; the orchestrator does, where `_submitted_recently` wakes the loop by accident and masked this). A live bridge emitting state but no events left the loop at ONE substrate tick after step 0 (Exp 60 trials, 2026-09-16: 120 probe windows, one tick each); the fake bridge's periodic "wind shifts" event masked it offline. The substrate submit cadence is now a wake source in its own right (`_substrate_tick_due`, also the substrate branch's own predicate). On that harness path the per-tick NAc decay now runs at the 0.5 s cadence (before: only on step 0); the orchestrator path is unchanged. Regression guard `tests/unit/test_substrate_primary_wake.py` (fake bridge with `events=False`, verified RED on the pre-fix loop). `FakeBridgeServer(events=...)` added, default `False` (the live-faithful condition: the real bridge emits text events only on chat/death).

### Changed
- **A GAINED modality's geometry tag now carries the declared range VALUES (H2 of the set-point-aware-neutral review, Option A — gained spaces only).** The tag hashed sensor names, normalization mode, dimension and gain, never the ranges — but a range is part of the `v → contribution` map, so `saturation` `[0,10] → [0,20]` (#726) changed the space while the tag stayed identical: persisted world nodes silently pattern-completed across it and the committed L11 replay re-printed different numbers with no mechanism change. `encode_sensors` and the EC's D66 migrate half now derive their tag fields through ONE helper (`similarity/encoder.py::sensor_geometry_fields`, discharging the recorded two-sites obligation); under gain it adds `ranges` (float-normalised); ungained tags are byte-identical (H3 golden). Encoder provenance records `declared_ranges` (last-write, `declared_ranges_mixed` when a session changes them; the migrate half refuses a mixed or unrecorded gained record). Consequence: every persisted WORLD node written before this change reads as a stale geometry on next load — masked with the existing one-line warning, removed by `maxim substrate invalidate --drop-geometry` (Minecraft sessions only; interoception/audio untouched). Red gate: `tests/unit/test_geometry_tag_ranges.py` (two range declarations → two tags + one mismatch warning; verified RED pre-change); golden world tag regenerated once for this field with every vector and ungained tag diffed identical. Residual (recorded trigger for widening): an ungained range re-declaration keeps the same hole. Ledger rows naming `SensorEncoder` discharged without re-stale (encode primitive byte-identical; no EARNED guard reuses persisted world substrate across the change).
- `simulation.minecraft_harness.run_minecraft_aut` / `_loop_kwargs` take an optional `substrate_telemetry` writer, forwarded only when given (the pinned kwargs are otherwise unchanged) — a harness can now see what the loop proposed per tick (Exp 60 chunk iii: 120 probe windows with zero executed actions and no telemetry to say why).
- `minecraft_player` body: `saturation` range `[0, 20]`, initial 10 (was `[0, 10]`, initial 5). Measured on the Exp 60 water classroom: the game never rests at 5 — a fed bot reads the bridge clamp 10 and a drained bot 0, both extremes, so the sensor carried a full-weight CONSTANT in every situation and lifted cos(shore, submerged) from the replayed 0.787 to 0.850 (> the 0.85 threshold). Rest is now the fed clamp at the midpoint; hunger drain descends loud (substrate-visible — saturation carries no drive, unlike `health`/`food`). Only this body declares saturation and no persisted substrate is re-encoded, but one committed record is re-measurable differently: `scripts/l11_real_trace_remeasure.py` re-reads body ranges, so the 2026-09-04 L11 re-measure now reads A4 0.0881/1.0/0.6852 (was 0.0566/0.9984/0.6309; same verdict; dated row in `docs/limits/l11_sensor_dilution.md`). Bridge fallback, the fake bridge, and rest fixtures moved to 10 accordingly.
