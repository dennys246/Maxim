...
class TestReadLearningSideEffects:
    """The learning-tier read, extracted from ``run_agentic_loop`` (item 16.4).

    Pins the behaviour that was inline before the move. The important
    property is the DEFAULT: a tool that reports nothing must yield
    "mechanical success means POSITIVE", because that is what every
    existing tool relies on — the D53 fix narrows the positive, it does
    not withdraw it.
    """
    ...
    def test_absent_side_effects_are_all_default(self):
        from maxim.runtime.agent_loop import LearningSideEffects, read_learning_side_effects
        ...
        # A result object with no side_effects attribute at all.
        assert read_learning_side_effects(object()) == LearningSideEffects()

    def test_reads_every_documented_key(self):
        from maxim.runtime.agent_loop import read_learning_side_effects
        ...
        assert got.outcome_ineffective is True
    ...
    def test_zero_drive_diff_survives_as_zero_not_none(self):
        """The measured-zero case is distinct from no-drive-sensor, and the
        distinction has to survive the read — ``record_outcome`` books
        nothing for the former and the tool-success floor for the latter."""
        from maxim.runtime.agent_loop import read_learning_side_effects
