...
class _ProxyHandler(BaseHTTPRequestHandler):
    ...
    def _check_auth(self) -> bool:
        ...
        if scheme.lower() == "bearer":
            ...
            if credential_ok:
                return True
            # Log ONLY the peer + a static reason — never key material, not
            # even a prefix or a hash of it. (Logging a prefix trips CodeQL
            # clear-text-logging; hashing the credential trips
            # weak-sensitive-data-hashing — an auth-failure log line simply
            # should not carry anything derived from the secret. Operators
            # diagnose key mismatches with `maxim doctor` / `maxim peer test`.)
            logger.warning(
                "Auth failed: peer=%s reason=invalid_bearer_credential",
                self.client_address[0],
            )
