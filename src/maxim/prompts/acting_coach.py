...
def _has_entity_tools(available_tools: set[str] | None) -> bool:
    ...
    Entity tools are auto-generated by ``embodiment/tool_bridge.py`` and
    follow the naming patterns ``{entity}_{affordance}`` (affordance tools)
    and ``read_{entity}_{sensor}`` (sensor read tools).  We detect them
    by checking for tools prefixed with ``read_`` (sensor) or any tool
    not in the known built-in set — since ``acting_coach`` is only set
    when ``entity_ref`` is provided, false positives here just produce
    slightly broader guidance (not harmful).
    """
