...
def _require_finite(value: Any, *, where: str) -> float:
    try:
        ...
        raise IngestRefused(duty="V2", reason=f"non-numeric value at {where}: {value!r}") from exc
    ...
    *,
    ...
    for evt_sig, link_list in links.items():
        ...
        for i, link in enumerate(link_list):
            ...
            deltas = list(td.get("observed_deltas", [])) if isinstance(td, dict) else []
            ...
            history = list(link.get("prediction_history", []) or [])
    ...
    for field_name, lo, hi, parts_expected in (
        ...
        for key, value in entries.items():
            if parts_expected:
                ...
                if field_name == "cluster_reward_bias" and "#" in parts[1]:
                    raise IngestRefused(
                        duty="V9",
                        reason=f"cluster id {parts[1]!r} carries a pre-crafted '#' collision suffix",
                    )
...
def _validate_ec_payload(
    ...
    *,
    ...
    for nid, node in ec_nodes.items():
        nid_s = str(nid)
        if NAC_KEY_SEP in nid_s:
            raise IngestRefused(duty="V9", reason=f"node id {nid_s!r} contains the NAC_KEY_SEP byte")
        if "#" in nid_s:
            raise IngestRefused(duty="V9", reason=f"node id {nid_s!r} carries a pre-crafted '#' collision suffix")
        ...
        for j, v in enumerate(embedding):
            _require_finite(v, where=f"{where}.embedding[{j}]")
    ...
    # arrays and compare with the manifest's stamp.
    ...
    if not isinstance(declared, dict) or {m: sorted(d) for m, d in sorted(declared.items())} != measured:
        raise IngestRefused(
            duty="V3",
            reason=(
    ...
    *,
    ...
    with zipfile.ZipFile(bundle_path, "r") as zf:
        ...
        if "nac" in declared_files:
            parsed = _loads_strict(zf.read(declared_files["nac"]), slice_name=declared_files["nac"])
            if not isinstance(parsed, dict):
                raise IngestRefused(duty="V2", reason="nac.json is not a JSON object")
            donor_nac = parsed
        if "ec" in declared_files:
            parsed = _loads_strict(zf.read(declared_files["ec"]), slice_name=declared_files["ec"])
    ...
    if donor_nac is not None:
        ...
        for key, valence in (donor_nac.get("percept_valences", {}) or {}).items():
            parts = str(key).split(NAC_KEY_SEP)
            if len(parts) == 3:
                valence_entries[parts[1]] = float(valence)
    ...
    result: SubstrateMergeResult = substrate_merge(
        receiver_nac=dict(receiver_nac or {}),
