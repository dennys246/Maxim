    ...
    def execute(self, affordance: str, params: dict[str, Any]) -> Any:
        ...
        try:
            ...
            metadata: dict[str, Any] = {
                ...
                # D53: the learning tier, as distinct from mechanical
                # success. Assert POSITIVE only on positive confirmation —
                # an unverified readback is NOT a success (unknown !=
                # achieved) — and key on the OUTCOME, never on
                # clamp-occurrence: a clamped turn that still moved and
                # reached its (clamped) target is a real turn and stays
                # POSITIVE. A command to where the body already was moved
                # nothing, so it teaches nothing.
                #
                # ``reached is False`` needs no entry here: this backend
                # already maps a confirmed shortfall to success=False, which
                # books NEGATIVE through the ordinary failure path. The
                # sibling producer (tools/reachy.py::FocusOnSoundTool)
                # returns success=True for that case and so has to emit
                # "negative" explicitly — same tier, two routes.
                #
                # Promoted onto ToolOutput.side_effects by
                # ModulatorAffordanceTool.execute; metadata alone is
                # structurally invisible to the learning chain.
                "outcome_valence": (
                    "neutral" if (reached is None or abs(target_body - current_body) <= _REACH_TOLERANCE_RAD) else None
                ),
            }
